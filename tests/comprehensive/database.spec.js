...
test.describe('Database Tests - Comprehensive', () => {
  ...
  test('User table operations work', async ({ page }) => {
    ...
    const table = page.locator(
      'table, .data-table, [data-testid="users-table"]'
    );
  ...
  test('Appointments table operations work', async ({ page }) => {
    ...
    const appointmentsTable = page.locator(
      '[data-testid="appointments-table"], .appointments-table, table'
    );
    if (await appointmentsTable.isVisible()) {
      ...
      const filterButton = page.locator(
        'button[data-testid="filter"], .filter-button, button:has-text("Filter")'
      );
  ...
  test('Medical records table operations work', async ({ page }) => {
    ...
    const recordsTable = page.locator(
      '[data-testid="records-table"], .medical-records-table, table'
    );
    if (await recordsTable.isVisible()) {
      ...
      const sortButton = page.locator(
        'button[data-testid="sort"], .sort-button, th[role="button"]'
      );
  ...
  test('Database backup and recovery work', async ({ page }) => {
    ...
    const backupButton = page.locator(
      'button:has-text("Backup"), [data-testid="backup-button"]'
    );
    if (await backupButton.isVisible()) {
      ...
      const successMessage = page.locator(
        '.success, .text-green-500, [role="alert"]'
      );
