...
test.describe('SESSIONS Module - Therapy and treatment sessions', () => {
  ...
  test.describe('API Endpoints', () => {
    test('sessions - GET request should return valid response', async ({
      request,
    }) => {
    ...
    test('sessions - POST request should create new record', async ({
      request,
    }) => {
      const testData = {
        name: 'Test Record',
    ...
    test('sessions - Invalid data should return 400 error', async ({
      request,
    }) => {
      const invalidData = {
    ...
    test('sessions - Unauthorized request should return 401', async ({
      request,
    }) => {
      const response = await request.get(`/api/sessions`, {
        headers: {
    ...
    test('sessions - Response should have proper structure', async ({
      request,
    }) => {
      const response = await request.get(`/api/sessions`);
      const data = await response.json();
    ...
    test('sessions - Sorting should work correctly', async ({ request }) => {
      const response = await request.get(
        `/api/sessions?sort=created_at&order=desc`
      );
      const data = await response.json();
  ...
  test.describe('Integration Tests', () => {
    ...
    test('Module should handle concurrent users', async ({ browser }) => {
      ...
      await Promise.all([page1.goto('/sessions'), page2.goto('/sessions')]);
