...
test.describe('Appointments Module - Comprehensive Tests', () => {
  ...
  test.describe('Appointment List View', () => {
    ...
    test('should show appointment information correctly', async ({ page }) => {
      ...
      await expect(appointmentCard.locator('[data-testid="appointment-type"]')).toBeVisible();
    });
    ...
    test('should sort appointments by status', async ({ page }) => {
      ...
      const statuses = await page.locator('[data-testid="status-badge"]').allTextContents();
    ...
    test('should show appointment count', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="appointment-count"]')).toContainText('2');
    });
  ...
  test.describe('Book New Appointment', () => {
    ...
    test('should book appointment successfully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Appointment booked successfully');
    ...
    test('should validate required fields', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="datetime-error"]')).toContainText('Date and time is required');
    });
    ...
    test('should validate date is not in the past', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="datetime-error"]')).toContainText('Date cannot be in the past');
    });
    ...
    test('should check for appointment conflicts', async ({ page }) => {
      ...
      const existingDateTime = new Date(existingAppointment!.scheduled_at).toISOString().slice(0, 16);
      ...
      await expect(page.locator('[data-testid="error-message"]')).toContainText('Doctor has a conflicting appointment');
    });
    ...
    test('should disable unavailable time slots', async ({ page }) => {
      ...
      const doctor = appointmentHelper.getTestDoctor(0);
      const existingAppointment = appointmentHelper.getTestAppointment(0);
      const existingDate = new Date(existingAppointment!.scheduled_at).toISOString().split('T')[0];
      const existingTime = new Date(existingAppointment!.scheduled_at).toTimeString().slice(0, 5);
      
      await page.selectOption('[data-testid="doctor-select"]', doctor!.id);
      await page.fill('[data-testid="date-input"]', existingDate);
      
      const timeSlot = page.locator(`[data-testid="time-slot-${existingTime}"]`);
    ...
    test('should show loading state during booking', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="booking-loading"]')).toBeVisible();
    });
    ...
    test('should cancel booking form', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="book-appointment-form"]')).not.toBeVisible();
    });
  });
  ...
  test.describe('Appointment Details and Management', () => {
    ...
    test('should confirm appointment', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Appointment confirmed');
      await expect(page.locator('[data-testid="appointment-status"]')).toContainText('مؤكد');
    });
    ...
    test('should cancel appointment', async ({ page }) => {
      ...
      await page.fill('[data-testid="cancellation-reason"]', 'Patient requested cancellation');
      await page.click('[data-testid="confirm-cancellation"]');
      
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Appointment cancelled');
      await expect(page.locator('[data-testid="appointment-status"]')).toContainText('ملغي');
    });
    ...
    test('should reschedule appointment', async ({ page }) => {
      ...
      await page.fill('[data-testid="reschedule-reason"]', 'Doctor requested reschedule');
      await page.click('[data-testid="confirm-reschedule"]');
      
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Appointment rescheduled');
    });
    ...
    test('should add appointment notes', async ({ page }) => {
      ...
      await page.fill('[data-testid="notes-input"]', 'Patient arrived 10 minutes late');
      await page.click('[data-testid="save-notes"]');
      
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Notes added successfully');
      await expect(page.locator('[data-testid="appointment-notes"]')).toContainText('Patient arrived 10 minutes late');
    });
    ...
    test('should mark appointment as completed', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Appointment completed');
      await expect(page.locator('[data-testid="appointment-status"]')).toContainText('مكتمل');
    });
  ...
  test.describe('Appointment Notifications', () => {
    ...
    test('should send appointment reminder', async ({ page }) => {
      ...
      await page.fill('[data-testid="reminder-message"]', 'Reminder: Your appointment is tomorrow');
      await page.click('[data-testid="send-reminder-button"]');
      
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Reminder sent successfully');
    });
    ...
    test('should send appointment confirmation', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Confirmation sent successfully');
    });
    ...
    test('should send appointment cancellation notice', async ({ page }) => {
      ...
      await page.fill('[data-testid="cancellation-message"]', 'Your appointment has been cancelled');
      await page.click('[data-testid="send-cancellation-button"]');
      
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Cancellation notice sent');
    });
  });
  ...
  test.describe('Accessibility Tests', () => {
    ...
    test('should have proper ARIA labels', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="appointment-card"]').first()).toHaveAttribute('aria-label');
    });
    ...
    test('should announce appointment changes to screen readers', async ({ page }) => {
      const appointment = appointmentHelper.getTestAppointment(0);
      await page.goto(`/appointments/${appointment!.id}`);
