...
test.describe('Insurance Module - Comprehensive Tests', () => {
  ...
  test.describe('Insurance Claims List View', () => {
    ...
    test('should show claim information correctly', async ({ page }) => {
      ...
      await expect(claimCard.locator('[data-testid="claim-number"]')).toContainText(firstClaim!.claim_number);
      await expect(claimCard.locator('[data-testid="claim-amount"]')).toContainText(firstClaim!.amount.toString());
      await expect(claimCard.locator('[data-testid="claim-status"]')).toBeVisible();
      await expect(claimCard.locator('[data-testid="insurance-provider"]')).toBeVisible();
    });
    ...
    test('should sort claims by amount', async ({ page }) => {
      ...
      const amounts = await page.locator('[data-testid="claim-amount"]').allTextContents();
    ...
    test('should sort claims by date', async ({ page }) => {
      ...
      const dates = await page.locator('[data-testid="claim-date"]').allTextContents();
    ...
    test('should show claim count', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="claim-count"]')).toContainText('2');
    });
  ...
  test.describe('Add New Insurance Claim', () => {
    ...
    test('should open add claim form', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="add-claim-form"]')).toBeVisible();
    });
    ...
    test('should create new claim successfully', async ({ page }) => {
      ...
      await page.selectOption('[data-testid="insurance-provider-select"]', 'التعاونية');
      await page.fill('[data-testid="policy-number-input"]', 'POL003');
      await page.selectOption('[data-testid="treatment-type-select"]', 'العلاج الطبيعي');
      await page.fill('[data-testid="diagnosis-input"]', 'إصابة الركبة');
      await page.fill('[data-testid="description-input"]', 'علاج طبيعي لإصابة الركبة اليسرى');
      
      await page.click('[data-testid="save-claim-button"]');
      
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Insurance claim created successfully');
    ...
    test('should validate required fields', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="claim-number-error"]')).toContainText('Claim number is required');
      await expect(page.locator('[data-testid="amount-error"]')).toContainText('Amount is required');
      await expect(page.locator('[data-testid="insurance-provider-error"]')).toContainText('Insurance provider is required');
    });
    ...
    test('should validate amount format', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="amount-error"]')).toContainText('Amount must be a valid number');
    });
    ...
    test('should validate claim number uniqueness', async ({ page }) => {
      ...
      await page.selectOption('[data-testid="patient-select"]', patient.id);
      await page.fill('[data-testid="claim-number-input"]', existingClaim!.claim_number);
      await page.fill('[data-testid="amount-input"]', '1000');
      await page.selectOption('[data-testid="insurance-provider-select"]', 'التعاونية');
      await page.click('[data-testid="save-claim-button"]');
      
      await expect(page.locator('[data-testid="error-message"]')).toContainText('Claim number already exists');
    });
    ...
    test('should cancel form without saving', async ({ page }) => {
      ...
      await expect(page).toHaveURL('/insurance');
      await expect(page.locator('[data-testid="add-claim-form"]')).not.toBeVisible();
    });
  });
  ...
  test.describe('Insurance Claim Details View', () => {
    ...
    test('should display claim details', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="claim-number"]')).toContainText(claim!.claim_number);
      await expect(page.locator('[data-testid="claim-amount"]')).toContainText(claim!.amount.toString());
    ...
    test('should show insurance information', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="insurance-provider"]')).toContainText(claim!.insurance_provider);
      await expect(page.locator('[data-testid="policy-number"]')).toContainText(claim!.policy_number);
    });
    ...
    test('should show treatment information', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="treatment-type"]')).toContainText(claim!.treatment_type);
      await expect(page.locator('[data-testid="diagnosis"]')).toContainText(claim!.diagnosis);
    });
    ...
    test('should show claim attachments', async ({ page }) => {
      ...
      for (const attachment of claim!.attachments) {
        await expect(page.locator('[data-testid="attachments-list"]')).toContainText(attachment);
      }
    });
  ...
  test.describe('Process Insurance Claim', () => {
    ...
    test('should approve claim', async ({ page }) => {
      ...
      await page.fill('[data-testid="approval-notes"]', 'تم الموافقة على المطالبة');
      await page.click('[data-testid="confirm-approval"]');
      
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Claim approved successfully');
      await expect(page.locator('[data-testid="claim-status"]')).toContainText('موافق عليه');
    });
    ...
    test('should reject claim', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Claim rejected successfully');
      await expect(page.locator('[data-testid="claim-status"]')).toContainText('مرفوض');
    });
    ...
    test('should request additional information', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="success-message"]')).toContainText('Information request sent');
    });
    ...
    test('should show approval confirmation dialog', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="approval-message"]')).toContainText(claim!.claim_number);
    });
    ...
    test('should show rejection confirmation dialog', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="rejection-message"]')).toContainText(claim!.claim_number);
    });
    ...
    test('should cancel rejection process', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="rejection-dialog"]')).not.toBeVisible();
    });
  });
  ...
  test.describe('Accessibility Tests', () => {
    ...
    test('should have proper ARIA labels', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="add-claim-button"]')).toHaveAttribute('aria-label');
      await expect(page.locator('[data-testid="claim-card"]').first()).toHaveAttribute('aria-label');
    });
    ...
    test('should support keyboard navigation', async ({ page }) => {
      ...
      await page.keyboard.press('Tab');
      await expect(page.locator('[data-testid="add-claim-button"]')).toBeFocused();
    });
