...
test.describe('Role Workflow Tests', () => {
  ...
  test('manager workflow - admin access but limited permissions', async ({ page }) => {
  ...
  test('workflow - logout clears session', async ({ page }) => {
    ...
    // Logout
    const logoutButton = page.locator('button:has-text("تسجيل الخروج"), button:has-text("logout"), [aria-label*="logout"]').first();
