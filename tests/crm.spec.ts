...
test.describe('CRM Contacts', () => {
  ...
  test('should create new contact', async({ page }) => {
    ...
    // ملء النموذج
    await page.fill('input[placeholder="أدخل الاسم الكامل"]', 'أحمد محمد العتيبي');
    await page.fill('input[placeholder="example@company.com"]', 'ahmed@example.com');
  ...
  test('should search contacts', async({ page }) => {
    await page.fill('input[placeholder="ابحث بالاسم أو البريد أو الشركة..."]', 'أحمد');
    await page.getByText('تطبيق الفلاتر').click();
...
test.describe('CRM Leads', () => {
  ...
  test('should create new lead', async({ page }) => {
    ...
    await page.fill('input[placeholder="example@company.com"]', 'info@healthcare.com');
    await page.fill('input[placeholder="0501234567"]', '0501234567');
...
test.describe('CRM Activities', () => {
  ...
  test('should filter activities by type', async({ page }) => {
    ...
    const meetingActivities = page.locator('.card').filter({ hasText: 'اجتماع' });
