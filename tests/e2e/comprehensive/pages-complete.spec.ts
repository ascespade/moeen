...
test.describe('Complete Page Tests - All Scenarios', () => {
  test.describe('Login Page - All Scenarios', () => {
    ...
    test('should toggle password visibility', async ({ page }) => {
      ...
      const showPasswordButton = page
        .locator('[aria-label*="show"], [aria-label*="عرض"]')
        .first();
  ...
  test.describe('Dashboard - All Scenarios', () => {
    ...
    test('should load dashboard layout', async ({ page }) => {
      ...
      const dashboardElement = page
        .locator('[data-testid="dashboard"], .dashboard, #dashboard')
        .first();
  ...
  test.describe('Patients Page - All Scenarios', () => {
    ...
    test('should display patients list or empty state', async ({ page }) => {
      ...
      const listOrEmpty = page
        .locator(
          '[data-testid="patients-list"], [class*="patient"], [class*="empty"]'
        )
        .first();
    ...
    test('should have add patient button', async ({ page }) => {
      ...
      const addButton = page
        .getByRole('button', { name: /إضافة|Add|New/i })
        .first();
    ...
    test('should filter patients by criteria', async ({ page }) => {
      ...
      const filterButton = page
        .locator('button:has-text("Filter"), button:has-text("تصفية")')
        .first();
  ...
  test.describe('Appointments Page - All Scenarios', () => {
    ...
    test('should display appointments calendar or list', async ({ page }) => {
      await page.waitForTimeout(2000);
      const appointmentsView = page
        .locator(
          '[data-testid="appointments"], [class*="appointment"], [class*="calendar"]'
        )
        .first();
      if (
        await appointmentsView.isVisible({ timeout: 5000 }).catch(() => false)
      ) {
    ...
    test('should allow creating new appointment', async ({ page }) => {
      ...
      const createButton = page
        .getByRole('button', { name: /موعد جديد|New Appointment|Create/i })
        .first();
  ...
  test.describe('Reports Page - All Scenarios', () => {
    ...
    test('should allow generating reports', async ({ page }) => {
      ...
      const generateButton = page
        .getByRole('button', { name: /Generate|إنشاء|توليد/i })
        .first();
      if (
    ...
    test('should export reports to PDF', async ({ page }) => {
      ...
      const exportButton = page
        .locator('button:has-text("PDF"), button:has-text("تصدير")')
        .first();
  ...
  test.describe('Settings Page - All Scenarios', () => {
    ...
    test('should allow profile updates', async ({ page }) => {
      ...
      const nameInput = page
        .locator('input[name="name"], input[name="full_name"]')
        .first();
    ...
    test('should handle language toggle', async ({ page }) => {
      ...
      const langToggle = page
        .locator(
          'button:has-text("EN"), button:has-text("AR"), select[name="language"]'
        )
        .first();
    ...
    test('should handle theme toggle', async ({ page }) => {
      ...
      const themeToggle = page
        .locator(
          'button:has-text("Dark"), button:has-text("مظلم"), [aria-label*="theme"]'
        )
        .first();
