...
test.describe('Database CRUD Operations - Comprehensive', () => {
  ...
  test.describe('Users Table Operations', () => {
    ...
    test('should update user data', async() => {
      let newName = `Updated User ${Date.now()}`
  ...
  test.describe('Data Integrity Tests', () => {
    test('should enforce unique constraints', async() => {
      let timestamp = Date.now();
      let duplicateEmail = `duplicate-${timestamp}@test.com`
  ...
  test.describe('Performance Tests', () => {
    test('should handle batch inserts efficiently', async() => {
      let timestamp = Date.now();
      let batchSize = 10;
      let patients = Array.from({ length: batchSize }, (_, i) => ({
        full_name: `Batch Patient ${timestamp}-${i}`
        national_id: `200${timestamp}${i}`
        phone: `+966${timestamp}${i}`
