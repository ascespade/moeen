...
test.describe('Module 6: Billing & Payments - الفواتير والمدفوعات', () => {
  ...
  test.describe('Phase 1: Database Tests', () => {
    ...
    test('1.2 - Database: Verify payments table exists', async() => {
      const data, error = await supabase
        .from('payments')
        .select('*')
        .limit(1);
    ...
    test('1.3 - Database: Verify payment_methods table exists', async() => {
      const data, error = await supabase
    ...
    test('1.4 - Database: Verify insurance table exists', async() => {
      const data, error = await supabase
        .from('insurance')
        .select('*')
        .limit(1);
  ...
  test.describe('Phase 3: API Tests', () => {
    test('3.1 - API: /api/billing endpoint', async({ request }) => {
      let endpoint = '/api/billing'.replace(':id', '123');
    ...
    test('3.2 - API: /api/payments endpoint', async({ request }) => {
      let endpoint = '/api/payments'.replace(':id', '123');
      let response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
    ...
    test('3.3 - API: /api/invoices endpoint', async({ request }) => {
      let endpoint = '/api/invoices'.replace(':id', '123');
      let response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
