...
test.describe('Module 4: Appointments - المواعيد', () => {
  let supabase: any;
  ...
  test.describe('Phase 1: Database Tests', () => {
    test('1.1 - Database: Verify appointments table exists', async () => {
    ...
    test('1.2 - Database: Verify appointment_types table exists', async () => {
      const { data, error } = await supabase
    ...
    test('1.3 - Database: Verify availability table exists', async () => {
      const { data, error } = await supabase
        .from('availability')
        .select('*')
        .limit(1);
  ...
  test.describe('Phase 2: UI Tests', () => {
    test('2.1 - UI: /dashboard/appointments page loads', async ({ page }) => {
    ...
    test('2.2 - UI: /dashboard/calendar page loads', async ({ page }) => {
      await page.goto('/dashboard/calendar').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
  ...
  test.describe('Phase 3: API Tests', () => {
    test('3.1 - API: /api/appointments endpoint', async ({ request }) => {
      const endpoint = '/api/appointments'.replace(':id', '123');
    ...
    test('3.2 - API: /api/appointments/:id endpoint', async ({ request }) => {
      const endpoint = '/api/appointments/:id'.replace(':id', '123');
      const response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
    ...
    test('3.3 - API: /api/availability endpoint', async ({ request }) => {
      const endpoint = '/api/availability'.replace(':id', '123');
      const response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
  ...
  test.describe('Phase 4: Feature Tests', () => {
    test('4.1 - Feature: Schedule', async ({ page }) => {
      await page.goto('/dashboard/appointments').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.2 - Feature: Reschedule', async ({ page }) => {
      await page.goto('/dashboard/appointments').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.3 - Feature: Cancel', async ({ page }) => {
      await page.goto('/dashboard/appointments').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
