...
test.describe('Module 8: Reports & Analytics - التقارير والتحليلات', () => {
  let supabase: any;
  ...
  test.describe('Phase 1: Database Tests', () => {
    test('1.1 - Database: Verify reports table exists', async () => {
    ...
    test('1.2 - Database: Verify report_templates table exists', async () => {
      const { data, error } = await supabase
    ...
    test('1.3 - Database: Verify analytics_data table exists', async () => {
      const { data, error } = await supabase
        .from('analytics_data')
        .select('*')
        .limit(1);
  ...
  test.describe('Phase 2: UI Tests', () => {
    test('2.1 - UI: /dashboard/reports page loads', async ({ page }) => {
    ...
    test('2.2 - UI: /dashboard/analytics page loads', async ({ page }) => {
      await page.goto('/dashboard/analytics').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
  ...
  test.describe('Phase 3: API Tests', () => {
    test('3.1 - API: /api/reports endpoint', async ({ request }) => {
      const endpoint = '/api/reports'.replace(':id', '123');
    ...
    test('3.2 - API: /api/analytics endpoint', async ({ request }) => {
      const endpoint = '/api/analytics'.replace(':id', '123');
      const response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
    ...
    test('3.3 - API: /api/stats endpoint', async ({ request }) => {
      const endpoint = '/api/stats'.replace(':id', '123');
      const response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
  ...
  test.describe('Phase 4: Feature Tests', () => {
    test('4.1 - Feature: Generate Reports', async ({ page }) => {
      await page.goto('/dashboard/reports').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.2 - Feature: Templates', async ({ page }) => {
      await page.goto('/dashboard/reports').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.3 - Feature: Statistics', async ({ page }) => {
      await page.goto('/dashboard/reports').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
