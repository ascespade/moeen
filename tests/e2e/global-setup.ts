...
async function setupTestData() {
  ...
  try {
    // Create test roles
    const testRoles = [
      { name: 'test_admin', permissions: ['admin', 'manage_users', 'system_config'] },
...
async function setupTestUsers() {
  ...
  try {
    ...
    for (const doctor of testDoctors) {
      await supabase
        .from('doctors')
        .upsert(doctor, { onConflict: 'email' });
    }
...
async function verifySetup() {
  ...
  try {
    ...
    const { data: roles } = await supabase.from('roles').select('*').eq('name', 'test_admin');
    if (!roles || roles.length === 0) {
      throw new Error('Test roles not created');
    }
    
    const { data: users } = await supabase.from('users').select('*').eq('email', 'test-admin@example.com');
    if (!users || users.length === 0) {
      throw new Error('Test users not created');
    }
    
    const { data: patients } = await supabase.from('patients').select('*').eq('email', 'patient1@example.com');
    if (!patients || patients.length === 0) {
      throw new Error('Test patients not created');
    }
    
    const { data: flows } = await supabase.from('chatbot_flows').select('*').eq('public_id', 'test-flow-1');
