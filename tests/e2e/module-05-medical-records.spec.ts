...
test.describe('Module 5: Medical Records - السجلات الطبية', () => {
  ...
  test.describe('Phase 2: UI Tests', () => {
    test('2.1 - UI: /dashboard/medical-records page loads', async ({
      page,
    }) => {
      await page
        .goto('/dashboard/medical-records')
    ...
    test('2.2 - UI: /dashboard/patients/[id]/records page loads', async ({
      page,
    }) => {
      await page
        .goto('/dashboard/patients/[id]/records')
        .catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
