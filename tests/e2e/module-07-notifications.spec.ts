...
test.describe('Module 7: Notifications - الإشعارات', () => {
  ...
  test.describe('Phase 2: UI Tests', () => {
    ...
    test('2.2 - UI: /dashboard/settings/notifications page loads', async ({
      page,
    }) => {
      await page
  ...
  test.describe('Phase 3: API Tests', () => {
    test('3.1 - API: /api/notifications endpoint', async ({ request }) => {
      const endpoint = '/api/notifications'.replace(':id', '123');
    ...
    test('3.2 - API: /api/notification-settings endpoint', async ({
      request,
    }) => {
      const endpoint = '/api/notification-settings'.replace(':id', '123');
      const response = await request
        .get(endpoint)
        .catch(() => ({ status: () => 404 }));
