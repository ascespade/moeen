...
async function loginUser(page: Page, email: string, password: string) {
  ...
  const submitButton = page
    .locator(
      'button[type="submit"], button:has-text("????"), button:has-text("?????")'
    )
    .first();
  ...
  await page.waitForURL(/\/(dashboard|admin|doctor|patient|staff|supervisor)/, {
    timeout: 15000,
  });
}
...
test.describe('E2E Tests - All User Types', () => {
  ...
  for (const userType of userTypes) {
    test(`? ${userType.role.toUpperCase()} - Login and Dashboard Access`, async ({
      page,
    }) => {
      try {
        // Login
        ...
        expect(currentURL).toContain(
          userType.dashboardPath.split('/')[1] || 'dashboard'
        );
        ...
        console.log(
          `? Test ${userType.role}: Login and Dashboard - FAILED - ${error.message}`
        );
        throw error;
      }
    });

    test(`? ${userType.role.toUpperCase()} - Dashboard Elements Visible`, async ({
      page,
    }) => {
      try {
        await loginUser(page, userType.email, userType.password);
        ...
        console.log(
          `? Test ${userType.role} Dashboard Elements - FAILED - ${error.message}`
        );
        throw error;
      }
    });

    test(`? ${userType.role.toUpperCase()} - API Authentication`, async ({
      page,
      request,
    }) => {
      try {
        ...
        console.log(
          `? Test ${userType.role} API Auth - PASSED (Status: ${apiResponse.status()})`
        );
      } catch (error) {
        console.log(
          `? Test ${userType.role} API Auth - FAILED - ${error.message}`
        );
        throw error;
      }
    });
  ...
  test('? Accessibility - ARIA Labels', async ({ page }) => {
    ...
    console.log(
      `  Total buttons: ${buttons}, With aria-label: ${buttonsWithAria}, Ratio: ${(ariaRatio * 100).toFixed(1)}%`
    );
