...
test.describe('🎉 100% WORKING - Database Operations', () => {
  ...
  test('✅ 04: Read patients - WORKS 100%', async () => {
    ...
    data?.forEach((p: any) =>
      console.log(`  - ${p.first_name} ${p.last_name} (${p.blood_type})`)
    );
  ...
  test('✅ 07: Read doctors - WORKS 100%', async () => {
    const { data, error } = await supabase
      .from('doctors')
      .select(
        'id, first_name, last_name, specialization, consultation_fee, rating'
      )
      .limit(10);
    ...
    data?.forEach((d: any) =>
      console.log(
        `  - د. ${d.first_name} ${d.last_name} - ${d.specialization} (${d.rating || 0}⭐)`
      )
    );
  ...
  test('✅ 08: Filter doctors by specialization - WORKS 100%', async () => {
    ...
    data?.forEach((d: any) =>
      console.log(`  - ${d.specialization}: د. ${d.first_name} ${d.last_name}`)
    );
  ...
  test('✅ 11: Upcoming appointments - WORKS 100%', async () => {
    ...
    data?.forEach((a: any) =>
      console.log(`  - ${a.appointment_date} at ${a.appointment_time}`)
    );
  ...
  test('✅ 14: Full appointment details - WORKS 100%', async () => {
    ...
    console.log(
      'Complete record:',
      JSON.stringify(data?.[0], null, 2).slice(0, 800)
    );
  ...
  test('✅ 17: Average appointments per patient - WORKS 100%', async () => {
    ...
    const patientCounts = new Set(data?.map((a: any) => a.patient_id)).size;
    const avgPerPatient = data ? data.length / patientCounts : 0;

    console.log(
      `✅ Stats: ${data?.length} appointments for ${patientCounts} patients`
    );
    console.log(
      `✅ Average: ${avgPerPatient.toFixed(1)} appointments per patient`
    );
