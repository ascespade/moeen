...
class TestRunner {
  ...
  async runTestSuite(suiteName) {
    return new Promise((resolve, reject) => {
      ...
      exec(command, { timeout: this.config.timeout }, async (error, stdout, stderr) => {
        if (error) {
          ...
          try {
            ...
            logger.info(`✅ Test suite ${suiteName} completed: ${results.passed || 0} passed, ${results.failed || 0} failed`);
            resolve();
          } catch (parseError) {
            logger.error(`❌ Failed to parse results for ${suiteName}:`, parseError);
            reject(parseError);
          }
        }
      });
  ...
  async generateHtmlReport() {
    try {
      ...
      await fs.writeFile(path.join(this.config.reportDir, 'test-report.html'), htmlContent);
  ...
  createHtmlReport() {
    const { suites, totalTests, passed, failed, skipped, duration } = this.results;
...
<body>
  ...
  async generateJsonReport() {
    try {
      ...
      await fs.writeFile(path.join(this.config.reportDir, 'test-results.json'), jsonContent);
  ...
  async generateSummaryReport() {
    try {
      const summary = {
        ...
        successRate: this.results.totalTests > 0 ? (this.results.passed / this.results.totalTests) * 100 : 0,
        suites: this.results.suites.map(suite => ({
          ...
          successRate: suite.totalTests > 0 ? (suite.passed / suite.totalTests) * 100 : 0
        }))
      };
      
      await fs.writeFile(path.join(this.config.reportDir, 'summary.json'), JSON.stringify(summary, null, 2));
  ...
  async analyzeResults() {
    ...
    try {
      ...
      logger.info(`   Duration: ${(this.results.duration / 1000).toFixed(2)} seconds`);
      
      // Analyze by suite
      logger.info(`📋 Suite Analysis:`);
      for (const suite of this.results.suites) {
        const suiteSuccessRate = suite.totalTests > 0 ? (suite.passed / suite.totalTests) * 100 : 0;
        logger.info(`   ${suite.name}: ${suite.passed}/${suite.totalTests} (${suiteSuccessRate.toFixed(2)}%)`);
      }
