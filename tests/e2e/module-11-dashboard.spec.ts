...
test.describe('Module 11: Dashboard & Stats - لوحة التحكم والإحصائيات', () => {
  let supabase: any;
  ...
  test.describe('Phase 1: Database Tests', () => {
    test('1.1 - Database: Verify dashboard_widgets table exists', async () => {
    ...
    test('1.2 - Database: Verify statistics table exists', async () => {
      const { data, error } = await supabase
        .from('statistics')
        .select('*')
        .limit(1);
    ...
    test('1.3 - Database: Verify kpis table exists', async () => {
      const { data, error } = await supabase
        .from('kpis')
        .select('*')
        .limit(1);
  ...
  test.describe('Phase 2: UI Tests', () => {
    test('2.1 - UI: /dashboard page loads', async ({ page }) => {
    ...
    test('2.2 - UI: /dashboard/overview page loads', async ({ page }) => {
      await page.goto('/dashboard/overview').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
  ...
  test.describe('Phase 3: API Tests', () => {
    test('3.1 - API: /api/dashboard endpoint', async ({ request }) => {
      const endpoint = '/api/dashboard'.replace(':id', '123');
    ...
    test('3.2 - API: /api/stats endpoint', async ({ request }) => {
    ...
    test('3.3 - API: /api/widgets endpoint', async ({ request }) => {
      const endpoint = '/api/widgets'.replace(':id', '123');
      const response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
  ...
  test.describe('Phase 4: Feature Tests', () => {
    test('4.1 - Feature: Overview', async ({ page }) => {
      await page.goto('/dashboard').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.2 - Feature: Quick Stats', async ({ page }) => {
      await page.goto('/dashboard').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.3 - Feature: Widgets', async ({ page }) => {
      await page.goto('/dashboard').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
