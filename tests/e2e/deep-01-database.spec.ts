...
test.describe('Deep Database Tests - Users Table', () => {
  ...
  test('DB-02: Inspect actual columns in users table', async () => {
    ...
    if (data && data[0]) {
      ...
      test
        .info()
        .annotations.push({ type: 'columns', description: columns.join(', ') });
  ...
  test('DB-05: Create user with correct column names', async () => {
    ...
    if (existingData && existingData[0]) {
      const actualColumns = Object.keys(existingData[0]);
      ...
      if (actualColumns.includes('full_name'))
        testUser.full_name = `Test User ${timestamp}`;
      if (actualColumns.includes('phone'))
        testUser.phone = `+966${timestamp}`.slice(0, 13);
  ...
  test('DB-07: Create patient with actual column names', async () => {
    ...
    if (sample && sample[0]) {
      ...
      if (columns.includes('full_name'))
        patient.full_name = `Patient ${timestamp}`;
      if (columns.includes('email'))
        patient.email = `patient${timestamp}@test.com`;
      if (columns.includes('phone'))
        patient.phone = `+966${timestamp}`.slice(0, 13);
      if (columns.includes('national_id'))
        patient.national_id = `1${timestamp}`.slice(0, 10);
      ...
      console.log(
        'Creating patient with actual columns:',
        Object.keys(patient)
      );
...
test.describe('Deep Database Tests - Appointments Table', () => {
  ...
  test('DB-14: Filter appointments by status', async () => {
    ...
    for (const status of statuses) {
      ...
      console.log(`Appointments with status '${status}':`, data);
    }
...
test.describe('Deep Database Tests - Complex Queries', () => {
  ...
  test('DB-20: Test data integrity - orphaned records', async () => {
    ...
    if (data && data.length > 0) {
      console.log(
        `Checking ${data.length} appointments for orphaned records...`
      );
