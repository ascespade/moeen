...
test.describe('Module 9: Settings & Configuration - الإعدادات والتكوين', () => {
  let supabase: any;
  ...
  test.describe('Phase 1: Database Tests', () => {
    test('1.1 - Database: Verify settings table exists', async () => {
    ...
    test('1.2 - Database: Verify system_config table exists', async () => {
      const { data, error } = await supabase
        .from('system_config')
        .select('*')
        .limit(1);
    ...
    test('1.3 - Database: Verify preferences table exists', async () => {
      const { data, error } = await supabase
        .from('preferences')
        .select('*')
        .limit(1);
  ...
  test.describe('Phase 2: UI Tests', () => {
    test('2.1 - UI: /dashboard/settings page loads', async ({ page }) => {
    ...
    test('2.2 - UI: Page has navigation', async ({ page }) => {
      await page.goto('/dashboard/settings').catch(() => page.goto('/dashboard'));
    ...
    test('2.3 - UI: Page is responsive', async ({ page }) => {
      await page.goto('/dashboard/settings').catch(() => page.goto('/dashboard'));
  ...
  test.describe('Phase 3: API Tests', () => {
    test('3.1 - API: /api/settings endpoint', async ({ request }) => {
      const endpoint = '/api/settings'.replace(':id', '123');
    ...
    test('3.2 - API: /api/config endpoint', async ({ request }) => {
      const endpoint = '/api/config'.replace(':id', '123');
      const response = await request.get(endpoint).catch(() => ({ status: () => 404 }));
  ...
  test.describe('Phase 4: Feature Tests', () => {
    test('4.1 - Feature: System Settings', async ({ page }) => {
      await page.goto('/dashboard/settings').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.2 - Feature: User Preferences', async ({ page }) => {
      await page.goto('/dashboard/settings').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
    ...
    test('4.3 - Feature: Clinic Info', async ({ page }) => {
      await page.goto('/dashboard/settings').catch(() => page.goto('/dashboard'));
      await page.waitForTimeout(2000);
