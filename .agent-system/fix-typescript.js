...
for (const file of apiFiles) {
  try {
    ...
    // Fix missing await
    content = content.replace(
      /const supabase\s*=\s*createClient\(\);(?!\s*const)/g,
...
try {
  ...
  content = content.replace(
    /export\s*{\s*requirePermission[^}]*}\s*from\s*['"]\.\/authorize['"];?/g,
    '// export { requirePermission } from "./authorize"; // Commented - not implemented'
  );
  content = content.replace(
    /export\s*{\s*requireAdmin[^}]*}\s*from\s*['"]\.\/authorize['"];?/g,
    '// export { requireAdmin } from "./authorize"; // Commented - not implemented'
  );
  content = content.replace(
    /export\s*{\s*AuthorizationCheck[^}]*}\s*from\s*['"]\.\/authorize['"];?/g,
    '// export { AuthorizationCheck } from "./authorize"; // Commented - not implemented'
  );
  content = content.replace(
    /export\s*\*\s*from\s*['"]\.\/rbac['"];?/g,
    '// export * from "./rbac"; // Commented - not implemented'
  );
