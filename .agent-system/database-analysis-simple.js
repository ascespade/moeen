...
async function analyzeDatabase() {
  ...
  try {
    ...
    try {
      ...
      if (healthError) {
        ...
        healthData.forEach(item => {
          // console.log(`  - ${item.tableName}: ${item.record_count} total, ${item.recent_records} recent`
        });
      }
    } catch (err) {
    ...
    try {
      const data: userstrings, error: rolesError = await supabase
        .from('users')
        .select('role')
        .not('role', 'is', null);
      ...
      if (rolesError) {
        ...
        let roleCounts = userstrings.reduce((acc, user) => {
    ...
    try {
      ...
      if (statusError) {
        ...
        let statusCounts = convStatuses.reduce((acc, conv) => {
    ...
    try {
      ...
      if (apptError) {
        ...
        let statusCounts = apptStatuses.reduce((acc, appt) => {
