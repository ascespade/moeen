...
// Configuration
let CONFIG = {
  MAX_ATTEMPTS: parseInt(process.env.MAX_ATTEMPTS_PER_MODULE, 10) || 6,
  TARGET_PERCENT: parseInt(process.env.MODULE_TARGET_PERCENT, 10) || 90,
  TIMEOUT_MS: parseInt(process.env.PLAYWRIGHT_TIMEOUT_MS, 10) || 60000,
  WORKERS: parseInt(process.env.PLAYWRIGHT_WORKERS_PER_MODULE, 10) || 2,
  MODULES: [
...
let logJSON = (obj) => {
  let jsonMsg = JSON.stringify({ time: timestamp(), ...obj });
  // console.log(jsonMsg);
...
function analyzeReport(reportPath) {
  try {
    ...
    let data = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
      ...
      for (const suite of suites) {
        if (suite.specs) {
          for (const spec of suite.specs) {
            total++;
            let allPassed = spec.tests && spec.tests.every(t => t.results && t.results.every(r => r.status === 'passed'));
            if (allPassed) {
              passed++;
            } else {
              failed++;
...
function runPlaywrightTest(module, testFile, outDir, attempt) {
  let reportFile = path.join(outDir, `report-attempt${attempt}.json`

  log(`[${module}] Running Playwright test (attempt ${attempt})`

  try {
    let cmd = `npx playwright test ${testFile} --config=playwright-auto.config.ts --workers=${CONFIG.WORKERS} --timeout=${CONFIG.TIMEOUT_MS}`
    ...
    let generatedReport = 'playwright-results.json';
...
function increaseTimeout() {
  ...
  try {
    let configPath = 'playwright-auto.config.ts';
...
async function testModule(module) {
  ...
  let outDir = path.join('test-results', `${module}-${Date.now()}`
  fs.mkdirSync(outDir, { recursive: true });

  logJSON({ module, status: 'started', attempt: 0 });

  let testFile = TEST_FILE_MAP[module];
  ...
  for (let attempt = 1; attempt <= CONFIG.MAX_ATTEMPTS; attempt++) {
    ...
    // Run test
    let reportFile = runPlaywrightTest(module, testFile, outDir, attempt);

    // Analyze
    let analysis = analyzeReport(reportFile);
    let analysisFile = path.join(outDir, `analysis-attempt${attempt}.json`
    ...
    if (analysis.percent >= CONFIG.TARGET_PERCENT) {
      let duration = ((Date.now() - moduleStart) / 1000).toFixed(2);
...
async function main() {
  ...
  for (const module of CONFIG.MODULES) {
    let result = await testModule(module);
    results.push(result);
  }

  // Aggregate results
  let totalDuration = ((Date.now() - startTime) / 1000).toFixed(2);
  let successful = results.filter(r => r.status === 'success').length;
  let failed = results.filter(r => r.status === 'failed').length;
  let skipped = results.filter(r => r.status === 'skipped').length;
  let avgPercent = results.reduce((sum, r) => sum + r.percent, 0) / results.length;
  ...
  // Save final report
  let finalReportPath = `test-reports/final-report-${Date.now()}.json`
