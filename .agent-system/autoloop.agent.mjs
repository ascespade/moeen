...
<<<<<<< Updated upstream:.agent-system/autoloop.agent.mjs
      
      // 1. إصلاح ملفات JSON المكسورة أولاً
      await this.fixJSONFiles();
      
      // 2. إصلاح الملفات المكسورة جداً
      await this.fixSeverelyBrokenFiles();
      
      // 3. استخدام Prettier لإصلاح معظم المشاكل
      await this.fixWithPrettier();
      
      // 4. استخدام ESLint لإصلاح المشاكل المتبقية
      await this.fixWithESLint();
      
      // 5. كشف وإصلاح missing class declarations
      await this.fixMissingClassDeclarations();
      
      // 6. كشف وإصلاح missing const/let/var declarations
      await this.fixMissingVariableDeclarations();
      
      // 7. كشف وإصلاح missing export statements
      await this.fixMissingExports();
      
      // 8. كشف وإصلاح broken imports
      await this.fixBrokenImports();
      
      // 9. إصلاح مشاكل الأقواس المفقودة
      await this.fixMissingBrackets();
      
      // 10. إصلاح مشاكل function declarations
      await this.fixFunctionDeclarations();
      
      // 11. إصلاح syntax errors بسيطة
...
>>>>>>> Stashed changes:autoloop.agent.mjs
  ...
  // إصلاح missing class declarations
  async fixMissingClassDeclarations() {
    try {
      console.log(`${colors.dim}🔧 إصلاح missing class declarations...${colors.reset}`);
      
      const files = await this.getAllTypeScriptFiles();
      let fixedCount = 0;
      
      for (const file of files) {
        const fixed = await this.fixFileMissingClasses(file);
        if (fixed) fixedCount++;
      }
      
      console.log(`${colors.green}✅ تم إصلاح ${fixedCount} ملف${colors.reset}`);
      
    } catch (error) {
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح missing class declarations: ${error.message}${colors.reset}`);
    }
  }

  // إصلاح missing variable declarations
  async fixMissingVariableDeclarations() {
    try {
      console.log(`${colors.dim}🔧 إصلاح missing variable declarations...${colors.reset}`);
      
      const files = await this.getAllTypeScriptFiles();
      let fixedCount = 0;
      
      for (const file of files) {
        const fixed = await this.fixFileMissingVariables(file);
        if (fixed) fixedCount++;
      }
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح missing variable declarations: ${error.message}${colors.reset}`);
    }
  }

  // إصلاح missing exports
  async fixMissingExports() {
    try {
      console.log(`${colors.dim}🔧 إصلاح missing exports...${colors.reset}`);
      
      const files = await this.getAllTypeScriptFiles();
      let fixedCount = 0;
      
      for (const file of files) {
        const fixed = await this.fixFileMissingExports(file);
        if (fixed) fixedCount++;
      }
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح missing exports: ${error.message}${colors.reset}`);
    }
  }

  // إصلاح broken imports
  async fixBrokenImports() {
    try {
      console.log(`${colors.dim}🔧 إصلاح broken imports...${colors.reset}`);
      
      const files = await this.getAllTypeScriptFiles();
      let fixedCount = 0;
      
      for (const file of files) {
        const fixed = await this.fixFileBrokenImports(file);
        if (fixed) fixedCount++;
      }
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح broken imports: ${error.message}${colors.reset}`);
    }
  }

  // إصلاح الملفات المكسورة جداً
  async fixSeverelyBrokenFiles() {
    try {
      console.log(`${colors.dim}🔧 إصلاح الملفات المكسورة جداً...${colors.reset}`);
      
      // البحث عن الملفات التي تحتوي على أخطاء خطيرة
      const brokenFiles = await this.findSeverelyBrokenFiles();
      let fixedCount = 0;
      
      for (const file of brokenFiles) {
        const fixed = await this.repairSeverelyBrokenFile(file);
        if (fixed) fixedCount++;
      }
      
      console.log(`${colors.green}✅ تم إصلاح ${fixedCount} ملف مكسور${colors.reset}`);
  ...
  // البحث عن الملفات المكسورة جداً
  async findSeverelyBrokenFiles() {
    try {
      const result = this.runCommand('npx tsc --noEmit --skipLibCheck 2>&1 | grep -E "(Declaration or statement expected|Identifier expected)" | head -20', { silent: true });
      const lines = result.split('\n').filter(line => line.includes('.ts'));
      const files = [];
  ...
  // إصلاح ملف مكسور جداً
  async repairSeverelyBrokenFile(filePath) {
    try {
      ...
      // إصلاح مشاكل الـ imports المكسورة
      content = this.fixBrokenImportStatements(content);
      
      // إصلاح مشاكل الـ function declarations
      content = this.fixBrokenFunctionDeclarations(content);
      
      // إصلاح مشاكل الـ brackets
      content = this.fixBrokenBrackets(content);
  ...
  // إصلاح import statements مكسورة
  fixBrokenImportStatements(content) {
    // إصلاح imports مكسورة في بداية الملف
  ...
  // إصلاح function declarations مكسورة
  fixBrokenFunctionDeclarations(content) {
    // إصلاح function declarations بدون أقواس
    content = content.replace(/export async function (\w+)\s*\(\s*([^)]*)\s*\)\s*{/g, 'export async function $1($2) {');
    
    // إصلاح function declarations مكسورة
    content = content.replace(/export async function (\w+)\s*\(\s*([^)]*)\s*\)\s*$/gm, 'export async function $1($2) {\n  // TODO: Implement function body\n}');
    
    return content;
  }

  // إصلاح brackets مكسورة
  fixBrokenBrackets(content) {
    // إصلاح missing closing brackets
  ...
  // إصلاح ملفات JSON المكسورة
  async fixJSONFiles() {
    try {
      console.log(`${colors.dim}🔧 إصلاح ملفات JSON المكسورة...${colors.reset}`);
      ...
      for (const file of jsonFiles) {
        try {
          const filePath = `/home/ubuntu/moeen/${file}`;
          if (await this.fileExists(filePath)) {
            const content = await fs.readFile(filePath, 'utf8');
            const fixed = this.fixJSONContent(content);
          ...
          console.warn(`${colors.yellow}⚠️ فشل في إصلاح ${file}: ${error.message}${colors.reset}`);
        }
      }
      
      console.log(`${colors.green}✅ تم إصلاح ${fixedCount} ملف JSON${colors.reset}`);
      
    } catch (error) {
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح ملفات JSON: ${error.message}${colors.reset}`);
    }
  }
  ...
  fixJSONContent(content) {
    try {
      // إصلاح مشاكل JSON شائعة
      content = content.replace(/,(\s*[}\]])/g, '$1'); // إزالة فواصل زائدة
      content = content.replace(/([^\\])\\([^\\\/bfnrt])/g, '$1\\\\$2'); // إصلاح escape characters
      content = content.replace(/(\w+):/g, '"$1":'); // إضافة quotes للـ keys
  ...
  // إصلاح مشاكل الأقواس المفقودة
  async fixMissingBrackets() {
    try {
      console.log(`${colors.dim}🔧 إصلاح مشاكل الأقواس المفقودة...${colors.reset}`);
      ...
      for (const file of files) {
        try {
          const content = await fs.readFile(file, 'utf8');
          const fixed = this.fixFileBrackets(content);
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح الأقواس: ${error.message}${colors.reset}`);
    }
  }

  // إصلاح أقواس ملف واحد
  fixFileBrackets(content) {
    // إصلاح missing closing brackets
    const openBrackets = (content.match(/\{/g) || []).length;
    const closeBrackets = (content.match(/\}/g) || []).length;
    ...
    // إصلاح missing closing parentheses
  ...
  // إصلاح مشاكل function declarations
  async fixFunctionDeclarations() {
    try {
      console.log(`${colors.dim}🔧 إصلاح مشاكل function declarations...${colors.reset}`);
      ...
      for (const file of files) {
        try {
          const content = await fs.readFile(file, 'utf8');
          const fixed = this.fixFileFunctionDeclarations(content);
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح function declarations: ${error.message}${colors.reset}`);
    }
  }

  // إصلاح function declarations في ملف واحد
  fixFileFunctionDeclarations(content) {
    // إصلاح function declarations بدون أقواس
    content = content.replace(/export async function (\w+)\s*\(\s*([^)]*)\s*\)\s*{/g, 'export async function $1($2) {');
    ...
    // إصلاح function declarations بدون export
    content = content.replace(/async function (\w+)\s*\(\s*([^)]*)\s*\)\s*{/g, 'async function $1($2) {');
  ...
  async fixSimpleSyntaxErrors() {
    try {
      console.log(`${colors.dim}🔧 إصلاح syntax errors بسيطة...${colors.reset}`);
      
      // استخدام Prettier لإصلاح معظم المشاكل
      await this.fixWithPrettier();
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح syntax errors: ${error.message}${colors.reset}`);
    }
  }

  // استخدام Prettier لإصلاح المشاكل
  async fixWithPrettier() {
    try {
      console.log(`${colors.dim}🎨 استخدام Prettier لإصلاح المشاكل...${colors.reset}`);
      
      // تشغيل Prettier على جميع ملفات TypeScript
      const files = await this.getAllTypeScriptFiles();
      let fixedCount = 0;
      
      for (const file of files) {
        try {
          // تشغيل Prettier على الملف
          const result = this.runCommand(`npx prettier --write "${file}"`, { silent: true });
          if (result) {
            fixedCount++;
          }
        } catch (error) {
          // تجاهل الأخطاء في الملفات المكسورة جداً
          console.warn(`${colors.yellow}⚠️ لا يمكن إصلاح ${file}: ${error.message}${colors.reset}`);
        }
      }
      
      console.log(`${colors.green}✅ تم إصلاح ${fixedCount} ملف باستخدام Prettier${colors.reset}`);
      
    } catch (error) {
      console.warn(`${colors.yellow}⚠️ خطأ في استخدام Prettier: ${error.message}${colors.reset}`);
    }
  }

  // استخدام ESLint لإصلاح المشاكل
  async fixWithESLint() {
    try {
      console.log(`${colors.dim}🔍 استخدام ESLint لإصلاح المشاكل...${colors.reset}`);
      
      // تشغيل ESLint مع --fix
      try {
        this.runCommand('npx eslint src --ext .ts,.tsx --fix', { silent: true });
        console.log(`${colors.green}✅ تم تشغيل ESLint --fix${colors.reset}`);
      } catch (error) {
        console.log(`${colors.yellow}⚠️ ESLint --fix لم ينجح، لكن هذا طبيعي${colors.reset}`);
      }
      
    } catch (error) {
      console.warn(`${colors.yellow}⚠️ خطأ في استخدام ESLint: ${error.message}${colors.reset}`);
    }
  }
...
<<<<<<< Updated upstream:.agent-system/autoloop.agent.mjs
      ...
      // فحص جودة الكود - يجب أن يمر بدون أخطاء
=======
...
>>>>>>> Stashed changes:autoloop.agent.mjs
      ...
      if (!qualityCheck.passed) {
        console.log(`${colors.red}❌ فشل فحص الجودة: ${qualityCheck.errorCount || 0} أخطاء${colors.reset}`);
        
        // محاولة الإصلاح مرة واحدة فقط
        console.log(`${colors.yellow}🔧 محاولة الإصلاح...${colors.reset}`);
        await this.fixErrors();
<<<<<<< Updated upstream:.agent-system/autoloop.agent.mjs
...
>>>>>>> Stashed changes:autoloop.agent.mjs
        ...
        if (!recheck.passed) {
          console.log(`${colors.red}❌ فشل الإصلاح - تم إلغاء الـ commit${colors.reset}`);
          console.log(`${colors.yellow}⚠️ يرجى إصلاح الأخطاء يدوياً${colors.reset}`);
          return false;
        }
      }
<<<<<<< Updated upstream:.agent-system/autoloop.agent.mjs
      
      // المتابعة بالـ commit
...
<<<<<<< Updated upstream:.agent-system/autoloop.agent.mjs
      ...
      // فحص TypeScript - يجب أن يمر بدون أخطاء
=======

      // فحص TypeScript
>>>>>>> Stashed changes:autoloop.agent.mjs
      try {
        const result = this.runCommand('npx tsc --noEmit --skipLibCheck 2>&1', { silent: true });
        ...
        if (errorCount === 0) {
          ...
          console.log(`${colors.red}❌ TypeScript: ${errorCount} errors${colors.reset}`);
          return { passed: false, checks, score: 0, errorCount };
        }
      } catch (error) {
...
<<<<<<< Updated upstream:.agent-system/autoloop.agent.mjs
      ...
      // يجب أن تمر جميع الـ checks
      const passed = Object.values(checks).every(Boolean);
      
      console.log(`${colors.cyan}📊 نتيجة فحص الجودة: ${passed ? 'PASSED' : 'FAILED'}${colors.reset}`);
      
      return { passed, checks, score: passed ? 100 : 0 };
...
>>>>>>> Stashed changes:autoloop.agent.mjs
  ...
  async getAllTypeScriptFiles() {
    try {
      const result = this.runCommand('find src -name "*.ts" -o -name "*.tsx" | head -50', { silent: true });
      return result.split('\n').filter(Boolean);
    } catch (error) {
      return [];
    }
  ...
  // إصلاح missing classes في ملف معين
  async fixFileMissingClasses(filePath) {
    try {
      if (!(await this.fileExists(filePath))) return false;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // البحث عن constructor بدون class
        if (line.match(/^\s*constructor\s*\(/)) {
          // التحقق من عدم وجود class declaration قبلها
          let hasClass = false;
          for (let j = i - 1; j >= 0; j--) {
          ...
          if (!hasClass) {
            // استخراج اسم الملف كاسم للـ class
            const className = this.getClassNameFromFile(filePath);
            lines.splice(i, 0, `export class ${className} extends Error {`);
            i++; // تخطي السطر المضاف
          }
        }
      }
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح missing classes لـ ${filePath}: ${error.message}${colors.reset}`);
      return false;
    }
  }

  // إصلاح missing variables في ملف معين
  async fixFileMissingVariables(filePath) {
    try {
      if (!(await this.fileExists(filePath))) return false;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // البحث عن object literals بدون declaration
        const match = line.match(/^\s*(\w+):\s*\{/);
        if (match) {
          // التحقق من عدم وجود declaration قبلها
          let hasDeclaration = false;
          for (let j = i - 1; j >= 0; j--) {
            if (lines[j].match(/^(const|let|var|export)/)) {
              hasDeclaration = true;
              break;
            }
          }
          ...
          if (!hasDeclaration) {
            // إضافة export const
            const varName = this.getVariableNameFromContext(lines, i);
            lines.splice(i, 0, `export const ${varName} = {`);
            i++; // تخطي السطر المضاف
          }
        }
      }
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح missing variables لـ ${filePath}: ${error.message}${colors.reset}`);
      return false;
    }
  }

  // إصلاح missing exports في ملف معين
  async fixFileMissingExports(filePath) {
    try {
      ...
      // البحث عن functions/classes بدون export
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        
        // البحث عن function declarations
        if (line.match(/^(function|const.*=.*function|const.*=.*=>)/) && !line.startsWith('export')) {
          lines[i] = line.replace(/^(function|const)/, 'export $1');
        }
        
        // البحث عن class declarations
        if (line.match(/^class\s+\w+/) && !line.startsWith('export')) {
          lines[i] = line.replace(/^class/, 'export class');
        }
      }
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح missing exports لـ ${filePath}: ${error.message}${colors.reset}`);
      return false;
    }
  }

  // إصلاح broken imports في ملف معين
  async fixFileBrokenImports(filePath) {
    try {
      if (!(await this.fileExists(filePath))) return false;
      ...
      if (uniqueImports.length !== importLines.length) {
        // إزالة duplicate imports
        let newLines = [];
        let inImports = true;
        
        for (const line of lines) {
          if (line.trim().startsWith('import')) {
            if (inImports && uniqueImports.includes(line)) {
              newLines.push(line);
              inImports = false;
            }
          } else {
      ...
      console.warn(`${colors.yellow}⚠️ خطأ في إصلاح broken imports لـ ${filePath}: ${error.message}${colors.reset}`);
      return false;
    }
  }

  // استخراج اسم الـ class من مسار الملف
  getClassNameFromFile(filePath) {
    const fileName = path.basename(filePath, path.extname(filePath));
    return fileName.charAt(0).toUpperCase() + fileName.slice(1);
  }

  // استخراج اسم المتغير من السياق
  getVariableNameFromContext(lines, index) {
  ...
  async generateDetailedReport() {
    try {
      console.log(`${colors.cyan}📊 إنشاء تقرير مفصل...${colors.reset}`);
      ...
      // جمع البيانات
      const tsResult = this.runCommand('npx tsc --noEmit --skipLibCheck 2>&1', { silent: true });
      report.errors.typescript = (tsResult.match(/error TS/g) || []).length;
      
      // فحص ESLint
      try {
        const eslintResult = this.runCommand('npm run lint:check 2>&1', { silent: true });
        report.errors.eslint = (eslintResult.match(/error|warning/g) || []).length;
      } catch (error) {
        report.errors.eslint = 0;
      }
      ...
      // فحص الأداء
      report.performance.score = await this.analyzePerformance();
      report.performance.largeFiles = await this.findLargeFiles();
      report.performance.duplicateCode = await this.findDuplicateCode();
      report.performance.unusedImports = await this.findUnusedImports();
      ...
      // حفظ التقرير
      await fs.writeFile('fix-report.json', JSON.stringify(report, null, 2));
      
      console.log(`${colors.green}✅ تم إنشاء التقرير: fix-report.json${colors.reset}`);
      
      return report;
      
    } catch (error) {
      console.warn(`${colors.yellow}⚠️ خطأ في إنشاء التقرير: ${error.message}${colors.reset}`);
      return null;
    }
  }
...
`;
  ...
  async analyzeRoutingConsistency() {
    return { issues: 0, details: {} };
  }
  ...
  async analyzeSchemaCompatibility() {
    return { issues: 0, details: {} };
  }
  async analyzeQueryOptimization() {
    return { issues: 0, details: {} };
  }
  async analyzeConnectionManagement() {
    return { issues: 0, details: {} };
  }
  async analyzeMigrationCompatibility() {
    return { issues: 0, details: {} };
  }
  async analyzeAPIEndpoints() {
    return { issues: 0, details: {} };
  }
  async analyzeResponseConsistency() {
    return { issues: 0, details: {} };
  }
  async analyzeAPIErrorHandling() {
    return { issues: 0, details: {} };
  }
  ...
  async analyzeDataValidationFlow() {
    return { issues: 0, details: {} };
  }
  async analyzeDataTransformation() {
    return { issues: 0, details: {} };
  }
  async analyzeDataPersistence() {
    return { issues: 0, details: {} };
  }
  async analyzeSecurityAuthentication() {
    return { issues: 0, details: {} };
  }
  async analyzeSecurityAuthorization() {
    return { issues: 0, details: {} };
  }
  async analyzeDataEncryption() {
    return { issues: 0, details: {} };
  }
  async analyzeSecurityInputValidation() {
    return { issues: 0, details: {} };
  }
