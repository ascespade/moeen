...
function fixSupabaseAPIs() {
  ...
  for (const file of files) {
    try {
      ...
      content = content.replace(/await\s+createClient[^(]/g, 'const supabase = createClient();\n  ');
...
function fixAuthExports() {
  ...
  try {
    ...
    content = content.replace(/import\s*{\s*requirePermission[^}]*}\s*from\s*['"]\.\/authorize['"]/g, '');
    content = content.replace(/import\s*{\s*requireAdmin[^}]*}\s*from\s*['"]\.\/authorize['"]/g, '');
    content = content.replace(/import\s*{\s*AuthorizationCheck[^}]*}\s*from\s*['"]\.\/authorize['"]/g, '');
    
    // Remove rbac import if missing
    content = content.replace(/export\s*\*\s*from\s*['"]\.\/rbac['"]/g, '');
    content = content.replace(/import\s*{[^}]*}\s*from\s*['"]\.\/rbac['"]/g, '');
