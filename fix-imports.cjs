...
function fixImports(filePath) {
  try {
    ...
    content = content.replace(/@\/components\/ui\/([A-Z][a-zA-Z]*)/g, '@/components/ui/$1'.toLowerCase());
    
    // Fix component names
    content = content.replace(/function __([A-Z][a-zA-Z]*)/g, 'function $1');
    content = content.replace(/export default function __([A-Z][a-zA-Z]*)/g, 'export default function $1');
