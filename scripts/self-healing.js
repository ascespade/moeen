...
class SelfHealingSystem {
  constructor() {
    ...
    this.healingRulesFile = path.join(this.workspaceRoot, 'config', 'healing-rules.json');
    this.healingHistoryFile = path.join(this.workspaceRoot, 'temp', 'healing-history.json');
    this.healingRules = {};
    this.healingHistory = [];
  }
  ...
  async createDefaultHealingRules() {
    ...
    fs.writeFileSync(this.healingRulesFile, JSON.stringify(this.healingRules, null, 2));
  ...
  async saveHealingHistory() {
    fs.writeFileSync(this.healingHistoryFile, JSON.stringify(this.healingHistory, null, 2));
  }
  ...
  async createDirectories() {
    ...
    const directories = [
      'temp', 'logs', 'reports', 'config', 'sandbox', 
      'learning', 'backups', 'n8n-workflows', 'videos'
    ];
    ...
    if (fs.existsSync(logDir)) {
      ...
      for (const file of files) {
        ...
        if (Date.now() - stats.mtime.getTime() > maxAge) {
          ...
          const backupPath = path.join(this.workspaceRoot, 'backups', `${file}.${Date.now()}`);
  ...
  async installDependencies() {
    ...
    return new Promise((resolve) => {
      exec('npm install', { cwd: this.workspaceRoot }, (error, stdout, stderr) => {
        if (error) {
  ...
  async runHealingCycle() {
    ...
    for (const error of errors) {
      ...
      if (rule && rule.successRate > 0.3) {
        ...
        this.log(`Skipping healing for ${error.category}.${error.rule} (low success rate)`, 'warn');
      }
    }
    
    this.log(`Healing cycle completed: ${healingResults.length} errors processed`);
    ...
    const report = {
      ...
      summary: {
        ...
        successRate: this.healingHistory.length > 0 ? 
          this.healingHistory.filter(h => h.success).length / this.healingHistory.length : 0
      }
    };
    
    const reportFile = path.join(this.workspaceRoot, 'reports', 'healing-report.json');
