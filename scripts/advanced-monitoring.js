...
class AdvancedMonitoring {
  constructor() {
    ...
    this.logFile = path.join(this.workspaceRoot, 'logs', 'advanced-monitoring.log');
    this.metricsFile = path.join(this.workspaceRoot, 'temp', 'monitoring-metrics.json');
  ...
  async analyzeMetrics(metrics) {
    ...
    const missingDirs = Object.entries(fsHealth).filter(([dir, exists]) => !exists);
  ...
  async processAlerts(alerts) {
    ...
    for (const alert of alerts) {
      this.log(`Alert [${alert.severity.toUpperCase()}]: ${alert.message}`, alert.severity);
  ...
  async fixFileSystemHealth() {
    ...
    const dirs = ['temp', 'logs', 'reports', 'config', 'sandbox', 'learning', 'backups'];
  ...
  async rotateLogFile(filename) {
    ...
    const backupPath = path.join(this.workspaceRoot, 'backups', `${filename}.${Date.now()}`);
  ...
  async generateMonitoringReport() {
    ...
    // Save report
    const reportFile = path.join(this.workspaceRoot, 'reports', 'monitoring-report.json');
