...
class MasterExecutor {
  constructor() {
    ...
    this.reportFile = path.join(this.workspaceRoot, 'reports', 'master-execution-report.json');
  ...
  async generateFinalReport() {
    ...
    report.summary.successfulModules = report.modules.filter(m => m.success).length;
    report.summary.failedModules = report.modules.filter(m => !m.success).length;
  ...
  async run() {
    try {
      ...
      // Log summary
      this.log(`Execution completed: ${report.summary.successfulModules}/${report.summary.totalModules} modules successful`);
      
      if (report.summary.failedModules > 0) {
        this.log(`Failed modules: ${report.modules.filter(m => !m.success).map(m => m.module).join(', ')}`);
      }
