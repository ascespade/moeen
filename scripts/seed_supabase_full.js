...
let supabase = () => ({} as any)(SUPABASE_URL, SUPABASE_KEY, {
...
async function insert(table, row) {
  ...
  if (error) {
    // console.error(`Insert error [${table}]`
    throw error;
  }
  return data;
}
...
let roles = [
  ...
  },
  {
    ...
    name: 'agent',
    display_name: 'Agent',
    permissions: { handle_conversations: true },
    is_system_role: false
  },
  {
    id: uuidv4(),
...
let flows = [
  ...
  },
  {
    ...
    response_template:
      'هذا موعدك القادم: {{appointment_date}} الساعة {{appointmentTime}} مع {{doctor_name}}',
    ai_model: 'gemini_pro',
    status: 'published'
  },
  {
    ...
    response_template:
      'هل تريد تأكيد إلغاء الموعد في {{appointment_date}} الساعة {{appointmentTime}}؟',
    ai_model: 'gemini_pro',
    status: 'published'
  }
];
...
let templates = [
  {
    ...
    category: 'greeting',
    language: 'ar',
    content: 'مرحبًا! أنا معين، مساعد مركز الهمم. كيف أقدر أساعدك اليوم؟'
  },
  {
    id: uuidv4(),
    ...
    language: 'ar',
    content: 'ما التاريخ والوقت المفضل لديك؟'
  },
  {
    id: uuidv4(),
    ...
    language: 'ar',
    content:
      'تم حجز الموعد بنجاح مع {{doctor_name}} بتاريخ {{date}} الساعة {{time}}. هل ترغب إشعار عبر الإيميل؟'
  }
];
...
let defaultSettings = [
  {
    public_id: 'site:branding',
    key: 'site:branding',
    value: {
      site_name: 'مركز الهمم',
      logo: 'logo.png',
  ...
  {
    public_id: 'chatbot:default',
    key: 'chatbot:default',
    value: { name: 'معين', language: 'ar', default_model: 'gemini_pro' },
    description: 'Chatbot core settings'
  },
  {
    public_id: 'notifications:defaults',
    key: 'notifications:defaults',
    value: { email_from: 'info@alhemmcenter.sa', telegram_group_id: null },
...
async function seed() {
  ...
  try {
    ...
    for (const r of roles) {
      ...
      const data: existingstring = await supabase
        .from('roles')
        .select('id')
        .eq('name', r.name)
        .maybeSingle();
      ...
      if (!existingstring) {
        ...
        // console.log(`string already exists: ${r.name}`
      }
    }
    ...
    let coreUsers = [
      {
        email: 'admin@alhemm.sa',
        password: 'AdminStrongPass!2025',
        name: 'مسؤول النظام',
        role: 'admin'
      },
      {
        email: 'manager@alhemm.sa',
        password: 'ManagerStrongPass!2025',
        name: 'مدير المركز',
        role: 'manager'
      }
    ];
    ...
    for (const cu of coreUsers) {
      ...
      if (!existingUser) {
        ...
        let passwordHash = await bcrypt.hash(cu.password, 10);
        let row = {
          id: uuidv4(),
          email: cu.email,
        ...
        // audit
        await audit(row.id, 'seed:create_user', 'user', row.id, null, {
          email: cu.email,
          role: cu.role
        });
      } else {
    ...
    // 3) strings assignment (user_roles)
    ...
    if (doctors && patients) {
      ...
      for (let i = 0; i < Math.min(patients.length, 5); i++) {
        ...
        let doctor = doctors[i % doctors.length];
        let apptDate = new Date(
          now.getTime() + (i + 1) * 24 * 60 * 60 * 1000
        );
        let dateStr = apptDate.toISOString().slice(0, 10); // YYYY-MM-DD
        let timeStr = `09:0${i}:00`
    ...
    {
      let title = 'مرحبًا من مركز الهمم';
    ...
    let waTpl = {
      ...
      components: [
        {
          type: 'BODY',
          text: 'تم تأكيد حجزك مع {doctor} بتاريخ {date} الساعة {time}'
        }
      ],
      usage_count: 0,
      is_active: true
    ...
    const data: adminData = await supabase
      ...
      .limit(1)
      .single();
    let adminId = adminData?.id || null;
    await audit(adminId, 'seed:complete', 'system', null, null, {
      seeded_at: new Date().toISOString()
    });

    // console.log('Seeding completed successfully.');
    // console.log(
      'Please review WhatsApp access_token in whatsapp_configs and replace it with the real token.'
    );
    // console.log(
      'Check Supabase Auth if you want real email-confirmation flows (consider creating Auth users via admin API).'
    );
  } catch (err) {
