...
class N8nWorkflowManager {
  ...
  async initialize() {
    ...
    const directories = ['n8n-workflows', 'config', 'logs', 'temp'];
        ...
        },
        {
          ...
          parameters: {
            ...
            body: {
              ...
              description:
                'Check out this amazing content! #healthcare #ai #innovation'
            }
          }
        },
        {
          ...
          parameters: {
            ...
            body: {
              ...
              caption:
                'Amazing healthcare innovation! #healthcare #ai #innovation'
            }
          }
        },
        {
  ...
  async validateWorkflow(workflow) {
    ...
    for (const node of workflow.nodes) {
      if (!node.id || !node.name || !node.type) {
        errors.push(
          `Node missing required properties: ${JSON.stringify(node)}`
        );
      }
        ...
        for (const connection of connections.main || []) {
          for (const link of connection) {
            if (
              ...
              errors.push(
                `Invalid connection from ${nodeId}: ${JSON.stringify(link)}`
              );
            }
          }
        }
  ...
  async runWorkflowDryTest(workflow) {
    ...
    try {
      ...
      this.log(
        `Dry test completed: ${testResults.nodes.length} nodes tested, ${testResults.errors.length} errors found`
      );
  ...
  async deployWorkflow(workflow) {
    ...
    try {
      ...
      this.log(
        `Deploying workflow "${workflow.name}" to ${config.n8n.baseUrl}`
      );
