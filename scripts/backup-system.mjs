...
async function backupFiles(files, reason = 'auto-backup') {
  ...
  try {
    ...
    try {
      ...
      const { stdout: branchName } = await executeCommand('git branch --show-current');
  ...
  try {
    ...
    for (const dir of backupDirs) {
      ...
      try {
        ...
        console.log(`     📊 ${metadata.files.length} ملف (${(metadata.totalSize / 1024).toFixed(2)} KB)`);
        console.log(`     🌿 ${metadata.gitBranch} (${metadata.gitCommit?.substring(0, 7)})`);
        console.log('');
      } catch (e) {
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.log('استخدام: node backup-system.mjs [backup|restore|list|clean]');
