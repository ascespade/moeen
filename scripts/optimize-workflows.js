...
function analyzeWorkflow(filePath) {
  ...
  for (const [jobName, job] of Object.entries(workflow.jobs || {})) {
    ...
    steps.forEach((step, index) => {
      ...
      if (step.uses) {
        if (
          step.uses.includes('actions/checkout@v4') ||
...
function generateOptimizationReport(analyses) {
  ...
  analyses.forEach(analysis => {
    ...
    // Metrics
    log(
      `  📊 Jobs: ${analysis.metrics.totalJobs}, Steps: ${analysis.metrics.totalSteps}`,
      'cyan'
    );
    log(
      `  ⏱️  Timeouts: ${analysis.metrics.hasTimeouts}/${analysis.metrics.totalJobs}`,
      'cyan'
    );
    log(`  🔄 Retries: ${analysis.metrics.hasRetries}`, 'cyan');
    log(`  💾 Caching: ${analysis.metrics.hasCaching}`, 'cyan');
    log(
      `  🔒 Permissions: ${analysis.metrics.hasPermissions ? 'Yes' : 'No'}`,
      'cyan'
    );
