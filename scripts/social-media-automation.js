...
class SocialMediaAutomation {
  constructor() {
    ...
    this.scheduleFile = path.join(this.workspaceRoot, 'temp', 'post-schedule.json');
    this.configFile = path.join(this.workspaceRoot, 'config', 'social-media-config.json');
  }
    ...
    if (!fs.existsSync(this.configFile)) {
      const defaultConfig = {
        ...
        posting: {
          ...
          hashtags: ['#healthcare', '#ai', '#innovation', '#technology', '#medical']
        },
        content: {
        ...
        if (stats.isFile()) {
          ...
          if (config.content.videoFormats.includes(ext)) {
            if (stats.size <= config.content.maxFileSize) {
              ...
              this.log(`Video too large: ${file} (${Math.round(stats.size / 1024 / 1024)}MB)`);
            }
          }
        }
      }
  ...
  async selectRandomVideos(count = 3) {
    ...
    while (selectedVideos.length < count && selectedVideos.length < availableVideos.length) {
  ...
  async generatePostSchedule() {
    ...
    const selectedVideos = await this.selectRandomVideos(config.posting.postsPerDay);
    ...
    for (let i = 0; i < selectedVideos.length; i++) {
      ...
      schedule.push({
        ...
        platforms: Object.keys(config.platforms).filter(platform => 
  ...
  async postToPlatform(platform, postData) {
    ...
    try {
      ...
      const result = await this.simulatePlatformPost(platform, postData, platformConfig);
  ...
  async generatePostingReport() {
    ...
    // Save report
    const reportFile = path.join(this.workspaceRoot, 'reports', 'social-media-report.json');
