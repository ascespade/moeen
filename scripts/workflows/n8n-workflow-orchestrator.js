...
class N8nWorkflowOrchestrator {
  ...
  async checkWorkflowHealth(workflowId, workflow) {
    try {
      ...
      const response = await this.apiClient.get(`/api/v1/workflows/${workflowId}`);
      ...
      if (response.status === 200) {
        ...
        if (deprecatedNodes.length > 0) {
          logger.warn(`⚠️ Workflow ${workflow.name} has deprecated nodes: ${deprecatedNodes.join(', ')}`);
        ...
        if (connectionIssues.length > 0) {
          logger.warn(`⚠️ Workflow ${workflow.name} has connection issues: ${connectionIssues.join(', ')}`);
          await this.fixConnectionIssues(workflowId, workflow, connectionIssues);
        }
        
      } else {
        workflow.status = 'unhealthy';
        logger.error(`❌ Workflow ${workflow.name} is unhealthy (status: ${response.status})`);
      }
  ...
  async fixDeprecatedNodes(workflowId, workflow, deprecatedNodes) {
    ...
    try {
      ...
      logger.error(`❌ Failed to fix deprecated nodes in workflow ${workflow.name}:`, error);
    }
  }
  ...
  async fixConnectionIssues(workflowId, workflow, issues) {
    ...
    try {
      ...
      logger.error(`❌ Failed to fix connection issues in workflow ${workflow.name}:`, error);
    }
  }
  ...
  async validateWorkflow(workflowId, workflow) {
    try {
      ...
      if (duplicateNames.length > 0) {
        validationResult.issues.push(`Duplicate node names: ${duplicateNames.join(', ')}`);
        validationResult.isValid = false;
      }
      ...
      if (!validationResult.isValid) {
        logger.warn(`⚠️ Workflow ${workflow.name} validation failed: ${validationResult.issues.join(', ')}`);
      }
  ...
  async checkPerformanceIssues() {
    ...
    try {
      ...
      const slowExecutions = executions.filter(exec => 
  ...
  async performPerformanceAnalysis() {
    ...
    try {
      ...
      logger.info(`📊 Performance analysis completed - Success rate: ${analysis.successRate.toFixed(2)}%`);
  ...
  async simulateWorkflowExecution(workflowId) {
    ...
    try {
      const response = await this.apiClient.post(`/api/v1/workflows/${workflowId}/execute`, {
        data: {},
        mode: 'simulation'
      });
      ...
      if (response.status === 200) {
        ...
        logger.error(`❌ Workflow ${workflowId} simulation failed: ${response.status}`);
        return false;
      }
  ...
  async saveStatistics() {
    try {
      ...
      await fs.writeFile('./logs/n8n-stats.json', JSON.stringify(statsData, null, 2));
