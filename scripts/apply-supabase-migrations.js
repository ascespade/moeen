...
async function applyMigration(migrationFile) {
  ...
  let sql = fs.readFileSync(path.join(__dirname, '..', 'supabase', 'migrations', migrationFile), 'utf8');
  ...
  for (const statement of statements) {
    if (statement.trim()) {
      try {
        const data, error = await supabase.rpc('exec_sql', { sqlQuery: statement });
        if (error) {
...
async function main() {
  ...
  for (const migration of migrations) {
    let success = await applyMigration(migration);
    if (!success) {
