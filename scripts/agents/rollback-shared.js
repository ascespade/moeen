...
class SharedRollback {
  ...
  async run() {
    try {
      console.log(`🔄 SharedRollback starting for session: ${this.timestamp}`);
  ...
  static async listSessions() {
    ...
    const sharedQuarantineDir = path.join(
      projectRoot,
      "src",
      ".shared_quarantine",
      "shared",
    ...
    try {
      const entries = await fs.readdir(sharedQuarantineDir, {
        withFileTypes: true,
      });
      ...
      for (const session of sessions) {
        const sessionDir = path.join(sharedQuarantineDir, session);
...
if (require.main === module) {
  ...
  if (args.includes("--list") || args.includes("-l")) {
    SharedRollback.listSessions().catch(console.error);
  } else if (args.length === 0) {
    console.log("Usage: node rollback-shared.js <timestamp>");
    console.log("       node rollback-shared.js --list");
    console.log("");
    console.log("Examples:");
    console.log("  node rollback-shared.js 2025-10-16T10-30-45-123Z");
    console.log("  node rollback-shared.js --list");
    process.exit(1);
  } else {
    const timestamp = args[0];
    const rollback = new SharedRollback(timestamp);
    rollback.run().catch(console.error);
  }
}

module.exports = SharedRollback;
