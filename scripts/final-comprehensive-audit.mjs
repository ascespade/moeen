...
async function checkBuildErrors() {
  ...
  try {
    ...
    console.log(stderr ? `??  Build completed with warnings` : `? Build successful - 0 errors`);
  } catch (error) {
    ...
    console.log(`? Build failed: ${errors.length} errors, ${warnings.length} warnings`);
  }
}
...
async function checkCodeQuality() {
  ...
  for (const file of files.slice(0, 100)) {
    ...
    functions.forEach(func => {
      ...
      const complexity = (func.match(/(if|for|while|switch)\s*\(/g) || []).length;
      if (complexity > 5) {
  ...
  if (totalAnyCount === 0 && totalLongFunctions === 0 && totalComplexFunctions === 0) {
    console.log(`? Code Quality: Excellent`);
  } else {
    console.log(`??  Code Quality: ${totalAnyCount} 'any' types, ${totalLongFunctions} long functions, ${totalComplexFunctions} complex functions`);
  }
}
...
async function checkGarbageFiles() {
  ...
  for (const pattern of garbagePatterns) {
    const files = await glob(pattern, { ignore: ['node_modules/**', '.git/**', '.next/**'] });
    garbageFiles.push(...files.slice(0, 20));
  }
  
  // Check for duplicate files
  const allFiles = await glob('src/**/*.{ts,tsx,js,jsx}', { ignore: ['**/node_modules/**'] });
...
async function checkBusinessLogic() {
  ...
  const workflowFiles = await glob('src/**/*{appointment,patient,medical,insurance,claim}*.{ts,tsx}');
  ...
  for (const file of workflowFiles.slice(0, 30)) {
    ...
    if (content.includes('appointment') && !content.includes('validate') && !content.includes('validation')) {
      missingValidation++;
    ...
    // Check authorization
    if (content.includes('patient') && content.includes('fetch') && !content.includes('auth') && !content.includes('authorize')) {
  ...
  if (missingValidation === 0 && missingConflictCheck === 0 && missingAuthorization === 0) {
    console.log(`? Business Logic: All workflows properly implemented`);
  } else {
    console.log(`??  Business Logic: ${missingValidation} missing validations, ${missingConflictCheck} missing conflict checks, ${missingAuthorization} missing authorizations`);
  }
}
...
async function checkWorkflowIntegration() {
  ...
  if (missingUtilities.length > 0) {
    ...
    for (const file of apiFiles.slice(0, 20)) {
      const content = fs.readFileSync(file, 'utf-8');
      if (content.includes('api-utils') || content.includes('createErrorResponse') || content.includes('createSuccessResponse')) {
        utilsUsage++;
      }
    }
...
function generateFinalReport() {
  ...
  const report = {
    ...
    status: totalErrors === 0 && totalWarnings === 0 ? 'EXCELLENT' : totalErrors === 0 ? 'GOOD' : 'NEEDS_IMPROVEMENT',
    summary: {
      totalErrors,
      totalWarnings,
      buildErrors: results.buildErrors.filter(e => e.severity === 'critical').length,
...
function generateRecommendations() {
  ...
  if (results.businessLogic.filter(b => b.severity === 'critical').length > 0) {
    recommendations.push({
      ...
      details: results.businessLogic.filter(b => b.severity === 'critical').map(b => b.issue)
    });
  }
  ...
  if (results.codeQuality.length > 0) {
    recommendations.push({
      ...
      recommendation: 'Improve code quality by replacing "any" types and breaking down long functions',
  ...
  if (results.garbageFiles.length > 0) {
    recommendations.push({
      ...
      details: [`${results.garbageFiles[0]?.count || 0} potential garbage files found`]
    });
  }
...
### Build & Compilation
${report.details.buildErrors.length > 0 ? `\n? **Issues Found:**\n${report.details.buildErrors.slice(0, 10).map(e => `- ${e.error}`).join('\n')}` : '\n? **No build errors**'}
...
### Code Quality
${report.details.codeQuality.length > 0 ? `\n??  **Issues Found:**\n${report.details.codeQuality.slice(0, 10).map(q => `- ${q.file}: ${q.issue}`).join('\n')}` : '\n? **Code quality is good**'}

### Business Logic
${report.details.businessLogic.length > 0 ? `\n??  **Issues Found:**\n${report.details.businessLogic.slice(0, 10).map(b => `- ${b.file}: ${b.issue}`).join('\n')}` : '\n? **Business logic properly implemented**'}
