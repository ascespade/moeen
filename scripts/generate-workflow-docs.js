...
function generateWorkflowMarkdown(analysis) {
  const { name, file, triggers, jobs, env, permissions, description } = analysis;
...
async function main() {
  ...
  analyses.forEach(analysis => {
    ...
    const docPath = join(projectRoot, 'docs', 'workflows', `${analysis.file.replace('.github/workflows/', '').replace(/\.(yml|yaml)$/, '')}.md`);
  ...
  log(`- Total jobs documented: ${analyses.reduce((sum, a) => sum + Object.keys(a.jobs).length, 0)}`, 'blue');
  log(`- Total steps documented: ${analyses.reduce((sum, a) => sum + Object.values(a.jobs).reduce((s, j) => s + j.steps.length, 0), 0)}`, 'blue');
