...
class AdminModule {
  constructor() {
    ...
    this.configFile = path.join(this.workspaceRoot, 'config', 'admin-config.json');
    this.reportFile = path.join(this.workspaceRoot, 'reports', 'admin-report.json');
  }
      ...
      const defaultConfig = {
        ...
        admin: {
          ...
          roles: {
            ...
            patient: {
              permissions: ['own_profile', 'own_appointments', 'own_medical_records'],
  ...
  async checkUserPermissions() {
    ...
    for (const user of config.admin.users) {
      ...
      if (user.permissions.includes('all')) {
        ...
        const rolePermissions = config.admin.roles[user.role]?.permissions || [];
        const invalidPermissions = user.permissions.filter(p => 
          !rolePermissions.includes(p) && p !== 'all'
        );
        
        if (invalidPermissions.length > 0) {
          userCheck.status = 'invalid';
          userCheck.issues.push(`Invalid permissions: ${invalidPermissions.join(', ')}`);
        }
      }
    ...
    this.log(`Permission checks completed for ${permissionChecks.length} users`);
  ...
  async validateRoleDefinitions() {
    ...
    const validPermissions = [
      'all', 'patients', 'appointments', 'medical_records', 'sessions',
      'therapy_notes', 'own_profile', 'own_appointments', 'own_medical_records',
      'linked_patient_profile', 'linked_appointments'
    ];
    ...
    for (const [roleName, roleConfig] of Object.entries(config.admin.roles)) {
      ...
      if (!roleConfig.permissions || !Array.isArray(roleConfig.permissions)) {
        ...
        const invalidPermissions = roleConfig.permissions.filter(p => 
          !validPermissions.includes(p)
        );
        
        if (invalidPermissions.length > 0) {
          roleCheck.status = 'invalid';
          roleCheck.issues.push(`Invalid permissions: ${invalidPermissions.join(', ')}`);
        }
      }
  ...
  async checkSecuritySettings() {
    ...
    if (passwordPolicy.minLength < 8) {
      ...
      securityCheck.passwordPolicy.issues.push('Minimum password length should be at least 8 characters');
    }
    
    if (!passwordPolicy.requireUppercase || !passwordPolicy.requireLowercase || 
        !passwordPolicy.requireNumbers || !passwordPolicy.requireSpecialChars) {
      securityCheck.passwordPolicy.status = 'warning';
      securityCheck.passwordPolicy.issues.push('Consider enabling all password complexity requirements');
    }
    
    // Check session settings
    const sessionTimeout = config.security.sessionTimeout;
    if (sessionTimeout > 7 * 24 * 60 * 60 * 1000) { // 7 days
      securityCheck.sessionSettings.status = 'warning';
      securityCheck.sessionSettings.issues.push('Session timeout is very long, consider reducing for security');
    }
    ...
    if (maxAttempts > 10) {
      ...
      securityCheck.loginSecurity.issues.push('Maximum login attempts is high, consider reducing');
    }
  ...
  async generateAdminReport() {
    ...
    report.summary.validUsers = report.userPermissions.filter(u => u.status === 'valid').length;
    report.summary.invalidUsers = report.userPermissions.filter(u => u.status === 'invalid').length;
    
    report.summary.totalRoles = report.roleDefinitions.length;
    report.summary.validRoles = report.roleDefinitions.filter(r => r.status === 'valid').length;
    report.summary.invalidRoles = report.roleDefinitions.filter(r => r.status === 'invalid').length;
    ...
    if (report.summary.invalidUsers === 0 && report.summary.invalidRoles === 0 && 
        report.supabaseConnection.success) {
      report.summary.overallStatus = 'healthy';
    } else if (report.summary.invalidUsers === 0 && report.summary.invalidRoles === 0) {
...
if (require.main === module) {
  ...
  adminModule.run()
    .then(report => {
      ...
      console.log(`Valid Users: ${report.summary.validUsers}/${report.summary.totalUsers}`);
      console.log(`Valid Roles: ${report.summary.validRoles}/${report.summary.totalRoles}`);
