...
async function introspectDatabase() {
  ...
  try {
    ...
    for (const tableName of DB_SCHEMA.tables) {
      ...
      if (sampleRows && sampleRows.length > 0) {
        ...
        if (!DB_SCHEMA.columns[tableName]) {
          DB_SCHEMA.columns[tableName] = Object.keys(sampleRows[0]).map(
            col => ({
              column_name: col,
              data_type: typeof sampleRows[0][col],
              is_nullable: sampleRows.some(row => row[col] === null)
                ? 'YES'
                : 'NO',
            })
          );
    ...
    console.log(
      `   - Total rows: ${Object.values(DB_SCHEMA.statistics).reduce((sum, s) => sum + s.row_count, 0)}`
    );
    console.log(
      `   - Tables with data: ${Object.values(DB_SCHEMA.statistics).filter(s => s.row_count > 0).length}\n`
    );
