...
class CursorAgentMonitor {
  ...
  async performHealthCheck() {
    ...
    if (systemMetrics.cpuUsage > this.config.cpuThreshold || 
      ...
      logger.warn(`⚠️ System resources high - CPU: ${systemMetrics.cpuUsage}%, Memory: ${systemMetrics.memoryUsage}%`);
    }
  ...
  async getSystemMetrics() {
    return new Promise((resolve) => {
      exec('ps -o pid,ppid,cmd,%cpu,%mem -p ' + Array.from(this.processes.values())
        .map(p => p.process.pid)
        .filter(pid => pid)
        .join(','), (error, stdout) => {
  ...
  async restartProcess(processName) {
    ...
    try {
      ...
      logger.info(`✅ Successfully restarted ${processName} (restart #${procInfo.restartCount})`);
  ...
  async collectAndStoreMetrics() {
    ...
    logger.info(`📊 Metrics collected - CPU: ${metrics.cpuUsage}%, Memory: ${metrics.memoryUsage}%, Status: ${metrics.status}`);
  }
  ...
  getHealthStatus() {
    return {
      ...
      processes: Array.from(this.processes.entries()).map(([name, info]) => ({
        ...
        uptime: info.process && !info.process.killed ? Date.now() - info.startTime : 0,
