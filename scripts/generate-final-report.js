...
class FinalReportGenerator {
  ...
  async loadExecutionReport() {
    let executionReportFile = path.join(
      this.reportsDir,
      'master-execution-report.json'
    );

    if (fs.existsSync(executionReportFile)) {
      let data = fs.readFileSync(executionReportFile, 'utf8');
  ...
  async loadFileCleanupReport() {
    let fileCleanupReportFile = path.join(
      this.reportsDir,
      'file-cleanup-report.json'
    );

    if (fs.existsSync(fileCleanupReportFile)) {
      let data = fs.readFileSync(fileCleanupReportFile, 'utf8');
      return JSON.parse(data);
    }
  ...
  async loadSocialMediaReport() {
    ...
    if (fs.existsSync(socialMediaReportFile)) {
      let data = fs.readFileSync(socialMediaReportFile, 'utf8');
      return JSON.parse(data);
    }
  ...
  async loadWorkflowTestResults() {
    let workflowTestFile = path.join(
      this.tempDir,
      'workflow-test-results.json'
    );

    if (fs.existsSync(workflowTestFile)) {
      let data = fs.readFileSync(workflowTestFile, 'utf8');
      return JSON.parse(data);
    }
  ...
  async getLogSummary() {
    ...
    for (const logFile of logFiles) {
      let logPath = path.join(this.logsDir, logFile);
...
if (require.main === module) {
  let generator = new FinalReportGenerator();
