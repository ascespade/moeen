...
class WorkflowTester {
  ...
  checkActAvailability() {
    try {
      ...
      console.warn(
        '⚠️ act is not available - install with: https://github.com/nektos/act'
      );
      return false;
    }
  }
  ...
  async testWorkflow(filename) {
    ...
    try {
      ...
      const jobLines = output
        .split('\n')
        .filter(line => line.includes('Job') && !line.includes('Workflow'));
      ...
      result.success = true;
      console.log(
        `  ✅ Workflow test passed (${result.jobs.length} jobs found)`
      );
    } catch (error) {
  ...
  async testWorkflowWithEvent(filename, eventType = 'push') {
    ...
    try {
      ...
      const output = execSync(
        `act --workflows ${workflowPath} --eventpath ${eventFile} --dryrun`,
        {
          encoding: 'utf8',
          stdio: 'pipe',
        }
      ...
      result.error = error.message;
      console.log(
        `  ❌ Workflow test with ${eventType} event failed: ${error.message}`
      );
    }
  ...
  createTestEvent(eventType) {
    ...
    fs.writeFileSync(
      eventFile,
      JSON.stringify(eventData[eventType] || {}, null, 2)
    );
    return eventFile;
  }
  ...
  async testSelfHealingWorkflow() {
    ...
    for (const testCase of testCases) {
      ...
      const result = await this.testWorkflowWithEvent(
        'ultimate-ci-self-healing.yml',
        testCase.event
      );
  ...
  generateTestReport() {
    ...
    fs.writeFileSync(
      'reports/workflow-test-report.json',
  ...
  async runComprehensiveTests() {
    ...
    fs.mkdirSync('reports', { recursive: true });
    fs.writeFileSync(
      'reports/comprehensive-test-report.json',
      JSON.stringify(report, null, 2)
    );
    ...
    console.log(
      `   Success rate: ${((report.summary.passedTests / allResults.length) * 100).toFixed(1)}%`
    );
