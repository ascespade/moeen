...
async function testAfterMigration() {
  ...
  const tables = [
    'users', 'patients', 'doctors', 'appointments', 'sessions', 'insurance_claims',
    'chatbot_flows', 'chatbot_nodes', 'chatbot_edges', 'chatbot_templates', 'chatbot_integrations',
    'conversations', 'messages', 'crm_leads', 'crm_deals', 'crm_activities',
    'notifications', 'internal_messages', 'audit_logs', 'roles', 'user_roles', 'settings'
  ];
  ...
  try {
    ...
    if (patientError) {
      ...
      console.log(`✅ Patient insertion test: Success (ID: ${patientData[0].id})`);
    ...
    if (flowError) {
      ...
      console.log(`✅ Chatbot flow insertion test: Success (ID: ${flowData[0].id})`);
