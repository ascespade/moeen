...
const conf = fs.existsSync('./ai_agent_config.json') ? JSON.parse(fs.readFileSync('./ai_agent_config.json','utf8')) : {};
...
async function callOpenAI(prompt) {
  ...
  const r = await client.chat.completions.create({
    model: 'gpt-4o-mini',
    messages: [{role:'system', content:'You are a senior test engineer.'}, {role:'user', content:prompt}],
    max_tokens: 1800
  });
...
(async ()=>{
  ...
  for (const mod of diff.modules || []) {
    ...
    if (provider === 'openai' && process.env.OPENAI_API_KEY) {
      try { content = await callOpenAI(prompt); } catch(e) { content = ''; }
    }
    if (!content) {
