...
const message = boxen(
  ...
  chalk.white('1. ') + chalk.cyan('node run.js') + chalk.gray(' - تشغيل كامل للمنصة\n') +
  chalk.white('2. ') + chalk.cyan('node core/settings-manager.js') + chalk.gray(' - إعداد المفاتيح\n') +
  chalk.white('3. ') + chalk.cyan('node settings.js') + chalk.gray(' - إعدادات المشروع\n') +
  chalk.white('4. ') + chalk.cyan('npm start') + chalk.gray(' - تشغيل عبر npm\n\n') +
  chalk.yellow('💡 نصائح:\n') +
  chalk.gray('• تأكد من تثبيت التبعيات أولاً: ') + chalk.white('npm install\n') +
  chalk.gray('• قم بإعداد المفاتيح قبل التشغيل\n') +
  chalk.gray('• الواجهة ستكون متاحة على: ') + chalk.white('http://localhost:3000'),
  {
    padding: 1,
    margin: 1,
    borderStyle: 'double',
...
if (!require('fs').existsSync(path.join(__dirname, 'node_modules'))) {
  ...
  install.on('close', (code) => {
    if (code === 0) {
      ...
      console.log(chalk.blue('🚀 يمكنك الآن تشغيل المنصة باستخدام: node run.js'));
    } else {
