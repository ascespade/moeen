...
```typescript
USER_ROLES = {
  ADMIN, DOCTOR, NURSE, STAFF, SUPERVISOR, 
  PATIENT, AGENT, MANAGER, DEMO
}
```
**⚠️ 9 أدوار**
...
type UserRole = 'patient' | 'doctor' | 'nurse' | 'staff' | 'agent' | 'supervisor' | 'manager' | 'admin' | 'demo';
```
...
export async function GET(request: NextRequest) {
  const authResult = await requirePermission(PERMISSIONS.APPOINTMENTS_READ)(request);
  if (!authResult.authorized) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }
  // ...
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const canAccess = await authorizeResourceAccess(user, 'appointment', params.id);
  if (!canAccess) {
