...
class ReportGenerator {
  ...
  async calculateStatistics() {
    ...
    if (this.reportData.candidates) {
      ...
      stats.files.removed +=
        this.reportData.candidates.orphanedFiles?.length || 0;
      stats.files.removed +=
        this.reportData.candidates.duplicateFiles?.length || 0;

      stats.dependencies.removed =
        this.reportData.candidates.unusedDependencies?.length || 0;
      stats.imports.dead = this.reportData.candidates.deadImports?.length || 0;

      // Calculate space saved
      let spaceSaved = 0;
      if (this.reportData.candidates.deadFiles) {
        spaceSaved += this.reportData.candidates.deadFiles.reduce(
          (sum, file) => sum + (file.size || 0),
          0
        );
      }
      if (this.reportData.candidates.orphanedFiles) {
        spaceSaved += this.reportData.candidates.orphanedFiles.reduce(
          (sum, file) => sum + (file.size || 0),
          0
        );
      }
      if (this.reportData.candidates.duplicateFiles) {
        spaceSaved +=
          this.reportData.candidates.duplicateFiles.reduce(
            (sum, file) => sum + (file.size || 0),
            0
          ) / 2;
      }
    ...
    if (this.reportData.dependencyGraph) {
      stats.files.total =
        this.reportData.dependencyGraph.stats?.totalFiles || 0;
      stats.imports.total =
        this.reportData.dependencyGraph.stats?.totalImports || 0;
      stats.dependencies.total = Object.keys(
        this.reportData.dependencyGraph.stats || {}
      ).length;
    }
