...
class DependencyGraphBuilder {
  ...
  getStats() {
    ...
    const totalImports = Array.from(this.imports.values()).reduce(
    ...
    return {
      ...
      averageImportsPerFile:
        totalFiles > 0 ? (totalImports / totalFiles).toFixed(2) : 0,
    };
  }
...
if (
  ...
  builder
    ...
    .then(result => {
      ...
      console.log(
        `Average imports per file: ${result.stats.averageImportsPerFile}`
      );
      console.log(`Unused modules: ${result.stats.unusedModules}`);

      if (result.stats.unusedModuleFiles.length > 0) {
        console.log('\n🗑️  Unused files:');
        result.stats.unusedModuleFiles.forEach(file =>
          console.log(`  • ${file}`)
        );
      }
