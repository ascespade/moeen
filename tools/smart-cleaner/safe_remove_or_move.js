...
class SafeRemoveOrMoveManager {
  ...
  async process(options = {}) {
    ...
    console.log(
      `🔧 Processing file removals and moves (${dryRun ? 'DRY RUN' : 'APPLYING CHANGES'})...`
    );

    if (!fs.existsSync(candidatesFile)) {
      console.error(
        '❌ Candidates file not found. Run identify_dead_files.js first.'
      );
      return;
    }
  ...
  async processDeadFiles(deadFiles) {
    ...
    for (const fileInfo of deadFiles) {
      ...
      if (this.dryRun) {
        console.log(
          `  📝 Would remove: ${fileInfo.file} (${this.formatFileSize(fileInfo.size)})`
        );
        ...
        try {
          ...
          console.error(
            `  ❌ Failed to remove ${fileInfo.file}:`,
            error.message
          );
        }
      }
  ...
  async processOrphanedFiles(orphanedFiles) {
    ...
    for (const fileInfo of orphanedFiles) {
      ...
      if (this.dryRun) {
        console.log(
          `  📝 Would remove orphaned: ${fileInfo.file} (${this.formatFileSize(fileInfo.size)})`
        );
        this.operations.push(operation);
      } else {
        try {
          ...
          console.error(
            `  ❌ Failed to remove orphaned ${fileInfo.file}:`,
            error.message
          );
        }
      }
  ...
  async processDuplicateFiles(duplicateFiles) {
    ...
    for (const duplicateInfo of duplicateFiles) {
      ...
      if (!fs.existsSync(duplicatePath)) {
        console.warn(
          `  ⚠️  Duplicate file not found: ${duplicateInfo.duplicate}`
        );
        continue;
      }
      ...
      if (this.dryRun) {
        console.log(
          `  📝 Would remove duplicate: ${duplicateInfo.duplicate} (original: ${duplicateInfo.original})`
        );
        this.operations.push(operation);
      } else {
        try {
          ...
          console.error(
            `  ❌ Failed to remove duplicate ${duplicateInfo.duplicate}:`,
            error.message
          );
        }
      }
  ...
  async processUnusedDependencies(unusedDependencies) {
    ...
    try {
      ...
      for (const depInfo of unusedDependencies) {
        ...
        if (this.dryRun) {
          console.log(
            `  📝 Would remove dependency: ${depInfo.name} (${depInfo.type})`
          );
          this.operations.push(operation);
        } else {
          try {
            ...
            console.error(
              `  ❌ Failed to remove dependency ${depInfo.name}:`,
              error.message
            );
          }
        }
  ...
  getSummary() {
    ...
    summary.totalSizeRemovedFormatted = this.formatFileSize(
      summary.totalSizeRemoved
    );
...
if (
  ...
  manager
    ...
    .then(result => {
      ...
      console.log(
        `Dependencies removed: ${result.summary.removedDependencies}`
      );
      console.log(
        `Total size removed: ${result.summary.totalSizeRemovedFormatted}`
      );
