...
class ImportPathsUpdater {
  ...
  async updateImportPaths(options = {}) {
    ...
    console.log(`🔗 Updating import paths (${dryRun ? 'DRY RUN' : 'APPLYING CHANGES'})...`);
  ...
  getSummary() {
    return {
      ...
      totalLinesChanged: this.changes.reduce((sum, change) => sum + change.changes.length, 0)
    };
  }
}
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  updater.updateImportPaths({ dryRun }).then(result => {
