...
async function loadConfig() {
  try {
    ...
    console.log('⚠️ لم يتم العثور على ملف التكوين، استخدام الإعدادات الافتراضية');
  }
}
...
async function analyzeDiff() {
  ...
  let files = [];
  try {
    const raw = execSync(`git diff --name-only ${from} ${to}`).toString().trim();
  ...
  console.log(`📊 تم تحليل ${files.length} ملف، ${uniqueModules.length} موديول متأثر`);
...
async function runTargetedTests(generatedTests) {
  ...
  for (const testFile of generatedTests) {
    try {
      ...
      results.failed++;
      results.tests.push({ file: testFile, status: 'failed', error: error.message });
    }
    results.total++;
  }
...
// إنشاء تقرير شامل
async function generateComprehensiveReport(diffMap, testResults, healingResults) {
  ...
  const reportFile = path.join(projectRoot, 'reports', 'intelligent-ci-report.json');
...
${cache.learned_patterns.slice(-5).map(p => `- **${p.module}**: ${p.details}`).join('\n')}
...
`;
  ...
  const mdReportFile = path.join(projectRoot, 'reports', 'intelligent-ci-report.md');
...
async function main() {
  ...
  try {
    ...
    // 7. إنشاء تقرير شامل
    const report = await generateComprehensiveReport(diffMap, testResults, healingResults);
    ...
    await logAIResult({
      ...
      qualityScore: Math.round((testResults.passed / testResults.total) * 100) || 0,
