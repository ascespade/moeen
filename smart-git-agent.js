...
class SmartGitAgent {
    ...
    async getUnmergedBranches() {
        ...
        try {
            ...
            const branchesOutput = await this.executeCommand('git branch -r --no-merged main');
            ...
            this.log(`Found ${branches.length} unmerged branches: ${branches.join(', ')}`);
            return branches;
        } catch (error) {
    ...
    shouldKeepLine(line, side, branchName) {
        ...
        const rules = [
            ...
            (line) => side === 'ours' && (filePath.includes('package.json') || filePath.includes('config')),
        ];
    ...
    async runOptimizations() {
        ...
        const optimizations = [
            ...
            },
            {
                ...
                command: 'npx eslint . --ext .js,.jsx,.ts,.tsx --fix --rule "no-unused-vars: error" || true',
    ...
    async runTests() {
        ...
        try {
            await this.executeCommand('npm test || npm run test || true', { throwOnError: false });
    ...
    async createFinalBranch() {
        ...
        try {
            await this.executeCommand('git add .');
            await this.executeCommand('git commit -m "Final optimized version with smart merges and optimizations"');
            await this.executeCommand('git checkout -B final-optimized-version');
            await this.executeCommand('git push origin final-optimized-version --force');
