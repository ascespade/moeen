...
function generateTestFile(module) {
  const testContent = `import { test, expect } from '@playwright/test';
import { realDB } from '@/lib/supabase-real';

test.describe('${module.name.toUpperCase()} Module - ${module.description}', () => {
  let testUser: any;
  let testPatient: any;
  let testDoctor: any;
  ...
  test.describe('API Endpoints', () => {
    ...
    test('${api} - POST request should create new record', async ({ request }) => {
      const testData = {
        name: 'Test Record',
        phone: '+966501234570',
        email: 'test@example.com'
    ...
    test('${api} - Invalid data should return 400 error', async ({ request }) => {
      const invalidData = {
    ...
    test('${api} - Unauthorized request should return 401', async ({ request }) => {
      const response = await request.get(\`/api/${api}\`, {
        headers: {
    ...
    test('${api} - Response should have proper structure', async ({ request }) => {
      const response = await request.get(\`/api/${api}\`);
      const data = await response.json();
    ...
    test('${api} - Pagination should work correctly', async ({ request }) => {
      const response = await request.get(\`/api/${api}?limit=10&offset=0\`);
      const data = await response.json();
    ...
    test('${api} - Search functionality should work', async ({ request }) => {
      const response = await request.get(\`/api/${api}?search=test\`);
      const data = await response.json();
    ...
    test('${api} - Filtering should work correctly', async ({ request }) => {
      const response = await request.get(\`/api/${api}?status=active\`);
      const data = await response.json();
    ...
    test('${api} - Sorting should work correctly', async ({ request }) => {
      const response = await request.get(\`/api/${api}?sort=created_at&order=desc\`);
      const data = await response.json();
    ...
    test('${api} - Error handling should be proper', async ({ request }) => {
  ...
  test.describe('User Interface', () => {
    ...
    test('${page} page should have proper navigation', async ({ page }) => {
      await page.goto(\`/${page}\`);
      await expect(page.locator('nav')).toBeVisible();
      await expect(page.locator('a[href="/dashboard"]')).toBeVisible();
    });

    test('${page} page should be responsive', async ({ page }) => {
      await page.goto(\`/${page}\`);
    ...
    test('${page} page should have proper accessibility', async ({ page }) => {
      await page.goto(\`/${page}\`);
    ...
    test('${page} page should handle loading states', async ({ page }) => {
      await page.goto(\`/${page}\`);
    ...
    test('${page} page should handle errors gracefully', async ({ page }) => {
      await page.goto(\`/${page}\`);
  ...
  test.describe('Integration Tests', () => {
    ...
    test('Module should integrate with database', async ({ page }) => {
      await page.goto('/${module.pages[0]}');
    ...
    test('Module should handle concurrent users', async ({ browser }) => {
      ...
      await Promise.all([
        page1.goto('/${module.pages[0]}'),
        page2.goto('/${module.pages[0]}')
      ]);
  ...
  test.describe('Performance Tests', () => {
    ...
    test('API should respond within acceptable time', async ({ request }) => {
      const startTime = Date.now();
      const response = await request.get('/api/${module.apis[0]}');
      const endTime = Date.now();
  ...
  test.describe('Security Tests', () => {
    ...
    test('Should prevent SQL injection', async ({ request }) => {
      ...
      const response = await request.post('/api/${module.apis[0]}', {
        data: maliciousData
      });
...
modules.forEach(module => {
  const testDir = path.join(__dirname, 'tests', 'modules', module.name);
