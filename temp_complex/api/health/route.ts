...
async function checkPerformance(): Promise<HealthCheck> {
  ...
  try {
    ...
    const isHealthy = metrics.averageResponseTime < 1000 && // Less than 1 second
    ...
    return {
      ...
      responseTime,
      message: isHealthy ? 'Performance metrics are normal' : 'Performance metrics indicate issues',
      details: metrics
    };
  } catch (error) {
    return {
...
async function checkExternalServices(): Promise<HealthCheck> {
  ...
  try {
    ...
    return {
      ...
      responseTime,
      message: allConfigured ? 'External services configured' : 'Some external services not configured',
      details: {
...
async function checkFileSystem(): Promise<HealthCheck> {
  ...
  try {
    ...
    return {
      ...
      responseTime,
      message: isHealthy ? 'File system accessible' : 'File system issues detected',
      details: {
