...
export const VALIDATION_MESSAGES = {
    ...
    INVALID_FILE_SIZE: (maxSize: string) => `File size must be less than ${maxSize}`,
    INVALID_IMAGE_DIMENSIONS: (width: number, height: number) => `Image must be ${width}x${height} pixels`,
    INVALID_IMAGE_ASPECT_RATIO: (ratio: string) => `Image aspect ratio must be ${ratio}`,
} as const;
