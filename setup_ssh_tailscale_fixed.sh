...
# 1. نسخ المفتاح العام إلى الخادم باستخدام scp مع كلمة المرور
echo "📋 نسخ المفتاح العام إلى الخادم..."
echo "root" | scp -o StrictHostKeyChecking=no public_key.txt ubuntu@100.64.64.33:~/

# 2. إعداد المفتاح على الخادم
echo "🔑 إعداد المفتاح على الخادم..."
sshpass -p "root" ssh -o StrictHostKeyChecking=no ubuntu@100.64.64.33 << 'EOF'
