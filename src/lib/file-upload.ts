...
export class FileUploadHandler {
  ...
  getMaxFileSize(fileType: string): number {
    switch (fileType) {
      ...
      case 'video': return MAX_FILE_SIZES.video;
      case 'medical': return MAX_FILE_SIZES.medical;
      default: return MAX_FILE_SIZES.documents;
    }
  }
...
export interface FileStorage {
  ...
  updateMetadata(fileId: string, updates: Partial<FileMetadata>): Promise<FileMetadata>;
}
...
export class LocalFileStorage implements FileStorage {
  ...
  async upload(file: File, metadata: Partial<FileMetadata>): Promise<FileMetadata> {
    // Validate file
    ...
    const validation = this.fileHandler.validateFile(file, allowedExtensions, maxSize);
    if (!validation.valid) {
    ...
    const fileName = this.fileHandler.generateFileName(file.name, metadata.uploadedBy || 'unknown');
...
export async function handleFileUpload(request: NextRequest): Promise<NextResponse> {
  try {
...
export async function handleFileDownload(fileId: string): Promise<NextResponse> {
  try {
    const storage = new LocalFileStorage();
...
export async function handleFileList(request: NextRequest): Promise<NextResponse> {
  try {
