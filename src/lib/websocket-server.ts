...
export interface SocketEvents {
  ...
  'patient:status_changed': (data: { patientId: string; status: string }) => void;
  
  // Appointment events
  'appointment:created': (data: { appointmentId: string; patientId: string; doctorId: string; date: string; time: string }) => void;
  'appointment:updated': (data: { appointmentId: string; changes: any }) => void;
  'appointment:cancelled': (data: { appointmentId: string; reason: string }) => void;
  'appointment:reminder': (data: { appointmentId: string; patientId: string; message: string }) => void;
  
  // Session events
  'session:started': (data: { sessionId: string; patientId: string; doctorId: string }) => void;
  'session:ended': (data: { sessionId: string; duration: number; notes: string }) => void;
  'session:exercise_completed': (data: { sessionId: string; exerciseId: string; patientId: string }) => void;
  
  // Conversation events
  'conversation:new_message': (data: { conversationId: string; patientId: string; message: string; type: string }) => void;
  'conversation:crisis_detected': (data: { patientId: string; message: string; severity: string }) => void;
  'conversation:ai_response': (data: { patientId: string; response: string; sentiment: string }) => void;
  
  // Notification events
  'notification:new': (data: { userId: string; type: string; title: string; message: string; priority: string }) => void;
  'notification:read': (data: { notificationId: string; userId: string }) => void;
  
  // Emergency events
  'emergency:alert': (data: { patientId: string; message: string; location: string; severity: string }) => void;
  'emergency:resolved': (data: { patientId: string; resolvedBy: string; notes: string }) => void;
  
  // System events
  'system:maintenance': (data: { message: string; duration: number }) => void;
  'system:performance_alert': (data: { metric: string; value: number; threshold: number }) => void;
}
...
export class WebSocketServer {
  ...
  private setupEventHandlers(): void {
    this.io.on('connection', (socket) => {
      ...
      socket.on('session:start', async (data: { patientId: string; doctorId: string }) => {
        const user = this.users.get(socket.id);
        if (user && user.role === 'doctor') {
      ...
      socket.on('conversation:message', async (data: { patientId: string; message: string; type: string }) => {
        const user = this.users.get(socket.id);
        if (user) {
          // Log conversation
      ...
      socket.on('crisis:detected', async (data: { patientId: string; message: string; severity: string }) => {
        // Log crisis
        await realDB.logConversation({
          patient_id: data.patientId,
          message_type: 'text',
      ...
      socket.on('notification:read', async (data: { notificationId: string }) => {
        ...
        if (user) {
          ...
          socket.emit('notification:read', { notificationId: data.notificationId, userId: user.id });
        }
      });
      ...
      socket.on('disconnect', () => {
        ...
        if (user) {
          ...
          if (user.role === 'patient') {
            socket.to(`patient:${user.id}`).emit('patient:left', { patientId: user.id });
          }
  ...
  private async authenticateUser(token: string, userId: string): Promise<SocketUser | null> {
    try {
  ...
  public async broadcastToPatient(patientId: string, event: string, data: any): Promise<void> {
    this.io.to(`patient:${patientId}`).emit(event, data);
  }

  public async broadcastToDoctor(doctorId: string, event: string, data: any): Promise<void> {
