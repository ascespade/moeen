...
export const PatientSchema = BaseEntitySchema.extend({
  ...
  phone: z.string().regex(/^(\+966|966|0)?[5-9][0-9]{8}$/).optional(),
...
export const ChatbotTemplateSchema = BaseEntitySchema.extend({
  name: z.string().min(1).max(255),
  category: z.enum(['greeting', 'appointment', 'information', 'support']).optional(),
...
export const CrmActivitySchema = BaseEntitySchema.extend({
  ...
  due_time: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional(),
...
export const ValidationHelpers = {
   ...
   */
  validateEntity<T>(schema: z.ZodSchema<T>, data: unknown): { success: boolean; data?: T; errors?: string[] } {
    try {
   ...
   */
  validateMultipleEntities<T>(schema: z.ZodSchema<T>, dataArray: unknown[]): {
    success: boolean;
    validData: T[];
