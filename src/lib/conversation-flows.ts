...
export class FlowManager {
      ...
      steps: [
        {
          id: 'welcome',
          type: 'information',
          content: 'أهلاً بك في مركز الهمم، أنا مُعين، مساعدك الرقمي. نحن هنا لتقديم الدعم لكل فرد.',
        ...
        },
        {
          id: 'collect_info',
          type: 'question',
          content: 'شكراً لك. هل يمكنك مشاركة بعض المعلومات الأساسية؟ (العمر، نبذة بسيطة عن التحدي)',
          nextStep: 'schedule_appointment'
        },
        {
          id: 'schedule_appointment',
          type: 'action',
          content: 'بناءً على المعلومات، الخطوة المثالية هي حجز موعد تقييم أولي مع أحد المختصين لدينا. هل تود أن أبحث لك عن أقرب موعد؟',
      ...
      steps: [
        {
          id: 'crisis_detection',
          type: 'action',
          content: 'أرى أنك قد تحتاج لمساعدة عاجلة. يرجى الاتصال فوراً بالرقم 997 أو 911.',
      ...
      steps: [
        {
          id: 'family_welcome',
          type: 'information',
          content: 'نحن نقدر دوركم كعائلة في رحلة الرعاية. كيف يمكنني مساعدتكم؟',
  ...
  getNextStep(flowId: string, currentStepId: string, userResponse?: string): FlowStep | undefined {
  ...
  // Evaluate condition
  private evaluateCondition(condition: FlowCondition, userResponse: string): boolean {
    switch (condition.operator) {
      case 'equals':
        return userResponse.toLowerCase() === condition.value.toLowerCase();
      case 'contains':
        return userResponse.toLowerCase().includes(condition.value.toLowerCase());
      case 'greater_than':
