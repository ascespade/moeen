...
export class ErrorHandler {
  ...
  static async handleError(error: any, request?: NextRequest): Promise<NextResponse> {
    // Log error
  ...
  private static async logError(error: any, request?: NextRequest): Promise<void> {
    try {
      const errorLog = {
...
export const createError = {
  ...
  externalAPI: (service: string, details?: any) => 
    new AppError(`${service} API error`, ErrorType.EXTERNAL_API_ERROR, 502, true, details),
