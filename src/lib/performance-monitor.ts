...
export class PerformanceMonitor {
  ...
  getStats(timeWindow?: number): PerformanceStats {
    ...
    const topErrorEndpoints = Array.from(endpointStats.entries())
      ...
      for (const metric of recentMetrics) {
        await realDB.logAudit({
          ...
          new_values: {
            ...
            memory_usage: {
              rss: metric.memoryUsage.rss,
              heapTotal: metric.memoryUsage.heapTotal,
              heapUsed: metric.memoryUsage.heapUsed,
              external: (metric.memoryUsage as any).external,
            },
            cpu_usage: {
              user: metric.cpuUsage.user,
              system: metric.cpuUsage.system,
            },
