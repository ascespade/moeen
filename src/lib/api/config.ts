...
export const getApiConfig = (): ApiConfig => {
  const whatsappToken = process.env.NEXT_PUBLIC_WHATSApp_TOKEN || process.env.WHATSAPP_TOKEN;
  const phoneNumberId = process.env.NEXT_PUBLIC_WHATSAPP_PHONE_NUMBER_ID || process.env.WHATSAPP_PHONE_NUMBER_ID;
  const webhookUrl = process.env.NEXT_PUBLIC_WHATSAPP_WEBHOOK_URL || process.env.WHATSAPP_WEBHOOK_URL;
