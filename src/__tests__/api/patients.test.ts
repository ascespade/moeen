...
describe('/api/patients', () => {
  ...
  describe('GET /api/patients', () => {
    it('should return patients with pagination', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/patients?page=1&limit=20&search=أحمد');
    ...
    it('should handle search functionality', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/patients?search=أحمد');
      const response = await GET(request);
    ...
    it('should handle database errors', async () => {
      mockRealDB.searchUsers.mockRejectedValue(new Error('Database connection failed'));
  ...
  describe('POST /api/patients', () => {
    it('should create a new patient successfully', async () => {
      ...
      expect(mockRealDB.createUser).toHaveBeenCalledWith(expect.objectContaining({
        name: 'أحمد محمد',
        age: 25,
        phone: '+966501234567',
        role: 'patient'
      }));
      expect(mockRealDB.createPatient).toHaveBeenCalledWith(expect.objectContaining({
        id: 'user-1',
        status: 'active'
      }));
    ...
    it('should handle WhatsApp API failures gracefully', async () => {
      ...
      mockRealDB.createUser.mockResolvedValue(mockUser);
      mockRealDB.createPatient.mockResolvedValue(mockPatient);
      mockWhatsappAPI.sendTextMessage.mockRejectedValue(new Error('WhatsApp API failed'));
