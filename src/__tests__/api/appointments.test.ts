...
describe('/api/appointments', () => {
  ...
  describe('GET /api/appointments', () => {
    it('should return appointments with filters', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/appointments?doctorId=doctor-1&date=2024-01-20');
    ...
    it('should filter by patient ID', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/appointments?patientId=patient-1');
      const response = await GET(request);
    ...
    it('should filter by status', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/appointments?status=completed');
      const response = await GET(request);
    ...
    it('should handle database errors', async () => {
      mockRealDB.getAppointments.mockRejectedValue(new Error('Database connection failed'));
  ...
  describe('POST /api/appointments', () => {
    it('should create a new appointment successfully', async () => {
      ...
      expect(mockRealDB.createAppointment).toHaveBeenCalledWith(expect.objectContaining({
        patient_id: 'patient-1',
        doctor_id: 'doctor-1',
        appointment_date: '2024-01-20',
        appointment_time: '10:00',
    ...
    it('should validate required fields', async () => {
      ...
      expect(data.success).toBe(false);
      expect(data.error).toBe('Patient ID, doctor ID, date, and time are required');
    });
    ...
    it('should handle WhatsApp API failures gracefully', async () => {
      ...
      mockRealDB.createAppointment.mockResolvedValue(mockAppointment);
      mockRealDB.getPatient.mockResolvedValue(mockPatient);
      mockWhatsappAPI.sendTemplateMessage.mockRejectedValue(new Error('WhatsApp API failed'));
    ...
    it('should handle database creation errors', async () => {
      ...
      mockRealDB.createAppointment.mockRejectedValue(new Error('Database error'));
  ...
  describe('PUT /api/appointments', () => {
    it('should update an appointment successfully', async () => {
      ...
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(mockRealDB.updateAppointment).toHaveBeenCalledWith('appointment-1', expect.objectContaining({
        appointment_date: '2024-01-21',
        appointment_time: '11:00'
      }));
    });
    ...
    it('should handle appointment not found', async () => {
      ...
      mockRealDB.updateAppointment.mockRejectedValue(new Error('Appointment not found'));
