...
describe('WhatsAppIntegration', () => {
  ...
  describe('Contact Management', () => {
    ...
    test('should update contact', () => {
      ...
      whatsapp.addContact(contact);
      const updated = whatsapp.updateContact('+966501234567', { name: 'أحمد محمد الأحمد' });
  ...
  describe('Message Processing', () => {
    test('should process text message', async () => {
      const response = await whatsapp.processIncomingMessage('+966501234567', 'مرحبا', 'text');
      expect(response).toContain('شكراً لك');
    });

    test('should process audio message', async () => {
      const response = await whatsapp.processIncomingMessage('+966501234567', '', 'audio');
      expect(response).toContain('رسالة صوتية');
    });

    test('should process image message', async () => {
      const response = await whatsapp.processIncomingMessage('+966501234567', '', 'image');
  ...
  describe('Template Messages', () => {
    ...
    test('should send motivational message', async () => {
      const result = await whatsapp.sendMotivationalMessage('+966501234567', 'أحمد');
      expect(result).toBeDefined();
    });

    test('should send milestone celebration', async () => {
      const result = await whatsapp.sendMilestoneCelebration('+966501234567', 'أحمد', 'إكمال 10 جلسات');
      expect(result).toBeDefined();
    });

    test('should send exercise reminder', async () => {
      const result = await whatsapp.sendExerciseReminder('+966501234567', 'أحمد', 'تمارين التنفس');
      expect(result).toBeDefined();
    });
  ...
  describe('Family Notifications', () => {
    test('should notify family member', async () => {
      const result = await whatsapp.notifyFamilyMember('+966501234568', 'أحمد', 'appointment', '10:00 صباحاً');
      expect(result).toBeDefined();
    });
  });
  ...
  describe('Accessibility Features', () => {
    ...
    test('should generate multilingual response', () => {
      ...
      const response = whatsapp.generateMultilingualResponse(arabicText, englishText, 'ar');
