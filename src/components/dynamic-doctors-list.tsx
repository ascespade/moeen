...
import { Stethoscope, Star, Clock, Phone, Mail, MapPin, User, RefreshCw, AlertTriangle } from 'lucide-react';
...
export default function DynamicDoctorsList({ 
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const response = await fetch('/api/dynamic-data/doctors?include_users=true');
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {doctor.rating > 0 && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm text-muted-foreground">({doctor.total_reviews} تقييم)</span>
                    </div>
                  )}
                </div>
                ...
                {doctor.working_hours && Object.keys(doctor.working_hours).length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold mb-2">ساعات العمل</h4>
                ...
                {doctor.user_info && (
                  <div className="mt-4 p-3 bg-muted rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-semibold">حساب المستخدم</span>
                    </div>
                    <div className="text-xs text-muted-foreground space-y-1">
                      <p><strong>الاسم:</strong> {doctor.user_info.user_name}</p>
                      <p><strong>الدور:</strong> {doctor.user_info.user_role}</p>
                      {doctor.user_info.last_login && (
                        <p><strong>آخر دخول:</strong> {new Date(doctor.user_info.last_login).toLocaleDateString('ar-SA')}</p>
                      )}
                    </div>
                  </div>
                )}
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span>عرض {filteredDoctors.length} من {doctors.length} طبيب</span>
              <span>التخصصات: {specializations.length} | اللغات: {languages.length}</span>
            </div>
          </CardContent>
        </Card>
      )}
