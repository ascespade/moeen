...
export default function __PreVisitChecklist({
  appointmentId,
  patientId,
  onChecklistComplete,
  ...
  const __handleItemToggle = async (_itemId: string, completed: boolean) => {
    setIsSaving(true);
  ...
  const __handleSubmitChecklist = async () => {
    setIsSaving(true);
  ...
  const __getCategoryColor = (_category: string) => {
    switch (category) {
      case "documents":
