...
export default function SystemConfigPanel() {
  ...
  const [activeTab, setActiveTab] = useState<
    'modules' | 'ai' | 'security' | 'automation'
  >('modules');
    ...
    <div className='max-w-6xl mx-auto p-6'>
      ...
      <div className='border-b border-gray-200 dark:border-gray-700 mb-6'>
        <nav className='-mb-px flex space-x-8'>
          {tabs.map(tab => (
            <button key={tab.id}
              onClick={() => { setActiveTab(tab.id as any) }} aria-label="??"
      ...
      {activeTab === 'modules' && (
        <div className='space-y-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
            {Object.entries(config.modules).map(
      ...
      {activeTab === 'ai' && (
        <div className='space-y-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            {Object.entries(config.ai_features).map(
                  ...
                  {'integrations' in featureConfig &&
                    featureConfig.integrations && (
                      <div className='space-y-2 mt-4'>
                        ...
                        <div className='flex flex-wrap gap-2'>
                          {featureConfig.integrations.map(
                            (integration, index) => (
                              <span
                                key={index}
              ...
              {Object.entries(config.security).map(([setting, enabled]) => (
                ...
                >
                  ...
                  <div className='flex items-center'>
                    ...
                    >
                      {enabled
                        ? t(I18N_KEYS.COMMON.ENABLED)
                        : t(I18N_KEYS.COMMON.DISABLED)}
                    </span>
                  </div>
                </div>
              ))}
