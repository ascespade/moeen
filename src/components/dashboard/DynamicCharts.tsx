...
export function DynamicLineChart({ data, title, color = '#2563eb' }: { 
  ...
  const points = data.map((d, i) => `${padding + i * xStep},${yScale(d.count)}`).join(' ');
...
// Bar Chart Component
export function DynamicBarChart({ data, title, color = '#10b981' }: { 
...
// Pie Chart Component
export function DynamicPieChart({ data, title }: { 
  data: Array<{ name: string; count: number }>; 
  title: string; 
}) {
  if (!data || data.length === 0) {
...
export function DynamicRecentActivities({ data }: { data: any }) {
  ...
  const getActivityColor = (type: string) => {
    switch (type) {
      ...
      case 'payment': return 'text-green-600 bg-green-50 dark:bg-green-900/20';
      case 'patient': return 'text-purple-600 bg-purple-50 dark:bg-purple-900/20';
