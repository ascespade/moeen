...
export const DashboardGrid: React.FC<DashboardGridProps> = ({
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const cellWidth =
        (gridRect.width - containerPadding[0] * 2 - margin[0] * (columns - 1)) /
        columns;
      const cellHeight = rowHeight + margin[1];

      const x = Math.floor(
        (e.clientX - gridRect.left - containerPadding[0]) /
          (cellWidth + margin[0])
      );
      const y = Math.floor(
        (e.clientY - gridRect.top - containerPadding[1]) /
          (cellHeight + margin[1])
      );
  ...
  const handleResizeMove = useCallback(
    (e: MouseEvent) => {
      ...
      const newWidth = Math.max(
        item.minWidth || 1,
        item.width + Math.floor(e.movementX / 100)
      );
      const newHeight = Math.max(
        item.minHeight || 1,
        item.height + Math.floor(e.movementY / 100)
      );

      if (onItemResize) {
        onItemResize(
          resizedItem,
          Math.min(newWidth, item.maxWidth || columns),
          Math.min(newHeight, item.maxHeight || 20)
        );
      }
    },
