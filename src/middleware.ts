...
import { authRateLimiter, apiRateLimiter, uploadRateLimiter } from '@/lib/rate-limiter';
...
export async function middleware(request: NextRequest) {
    ...
    if (!authRateLimiter.isAllowed(ip)) {
      return NextResponse.json(
        ...
        { 
          status: 429,
          headers: {
            'Retry-After': Math.ceil(authRateLimiter.getResetTime(ip) / 1000).toString(),
            'X-RateLimit-Limit': '5',
            'X-RateLimit-Remaining': authRateLimiter.getRemainingRequests(ip).toString(),
    ...
    if (!uploadRateLimiter.isAllowed(ip)) {
      return NextResponse.json(
        ...
        { 
          status: 429,
          headers: {
            'Retry-After': Math.ceil(uploadRateLimiter.getResetTime(ip) / 1000).toString(),
            'X-RateLimit-Limit': '10',
            'X-RateLimit-Remaining': uploadRateLimiter.getRemainingRequests(ip).toString(),
      ...
      return NextResponse.json(
        ...
        { 
          ...
          headers: {
            ...
            'X-RateLimit-Remaining': apiRateLimiter.getRemainingRequests(ip).toString(),
  ...
  if (pathname.startsWith('/api/')) {
    ...
    response.headers.set('X-RateLimit-Remaining', apiRateLimiter.getRemainingRequests(ip).toString());
    response.headers.set('X-RateLimit-Reset', apiRateLimiter.getResetTime(ip).toString());
  }
