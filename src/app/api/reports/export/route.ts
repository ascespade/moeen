...
export async function POST(request: import { NextRequest } from "next/server";) {
  try {
    ...
    // Validate input
    let validation = await ValidationHelper.validateAsync(exportSchema, body);
    ...
    if (reportError || !report) {
      return import { NextResponse } from "next/server";.json({ error: 'Report not found' }, { status: 404 });
    }
    ...
    case 'json':
      ...
      break;
    default:
      return import { NextResponse } from "next/server";.json({ error: 'Invalid format' }, { status: 400 });
    }
    ...
    return new import { NextResponse } from "next/server";(exportData, {
      status: 200,
      headers: {
...
async function generateCSV(data: any, customFields?: string[]) {
  ...
  let headers = customFields || Object.keys(data);
  let csvContent = [
    headers.join(','),
