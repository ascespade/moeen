...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRevenue = payments?.filter((p: any) => p.status === 'completed')
      .reduce((sum: any, p: any) => sum + parseFloat(p.amount.toString()), 0) || 0;
    ...
    const approvedClaims = claims?.filter((c: any) => c.claim_status === 'approved').length || 0;
    const pendingClaims = claims?.filter((c: any) => c.claim_status === 'pending' || c.claim_status === 'submitted').length || 0;
    const rejectedClaims = claims?.filter((c: any) => c.claim_status === 'rejected').length || 0;
