...
export async function __POST(_request: NextRequest) {
  try {
    ...
    const __tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);

    const __appointmentDate = new Date().toISOString().split("T")[0];
    ...
    // إنشاء رمز تأكيد
    const __confirmationCode = `APT${Date.now().toString().slice(-6)}`;
...
async function __sendWhatsAppConfirmation(_phone: string, appointment: unknown) {
  ...
  const __message = `تم حجز موعدك بنجاح!
