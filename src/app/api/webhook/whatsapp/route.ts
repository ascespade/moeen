...
export async function __POST(_request: NextRequest) {
  try {
    ...
    // التحقق من صحة الطلب
    const __verifyToken = request.headers.get("x-verify-token");
...
export async function __GET(_request: NextRequest) {
  ...
  const __mode = searchParams.get("hub.mode");
  const __token = searchParams.get("hub.verify_token");
  const __challenge = searchParams.get("hub.challenge");
...
async function __processWhatsAppMessage(_message: unknown, value: unknown) {
  try {
    const __phoneNumber = message.from;
    const __messageText = message.text?.body || "";
    const __messageId = message.id;
...
async function __processMessageWithAI(
...
async function __handleAppointmentIntent(
  conversationId: string,
  messageText: string,
  phoneNumber: string,
): Promise<string> {
...
async function __handleCancelIntent(
  conversationId: string,
  messageText: string,
  phoneNumber: string,
): Promise<string> {
...
async function __handleReminderIntent(
  conversationId: string,
  messageText: string,
  phoneNumber: string,
): Promise<string> {
...
async function __sendWhatsAppMessage(_phoneNumber: string, message: string) {
  try {
