...
const __webhookSecret = process.env.STRIPE_WEBHOOK_SECRET!;

export async function __POST(_request: NextRequest) {
  try {
    const __body = await request.text();
    const __signature = headers().get("stripe-signature");
...
async function __handlePaymentSucceeded(
  paymentIntent: Stripe.PaymentIntent,
  supabase: unknown,
) {
  const __appointmentId = paymentIntent.metadata?.appointmentId;
...
async function __handlePaymentFailed(
  paymentIntent: Stripe.PaymentIntent,
  supabase: unknown,
) {
  const __appointmentId = paymentIntent.metadata?.appointmentId;
...
async function __handlePaymentCanceled(
  paymentIntent: Stripe.PaymentIntent,
  supabase: unknown,
) {
  const __appointmentId = paymentIntent.metadata?.appointmentId;
