...
async function activatePatient(request: NextRequest, body: any) {
  try {
    ...
    // Validate input
    const validation = await ValidationHelper.validateAsync(activationSchema, body);
...
async function updateChecklist(request: NextRequest, body: any) {
  try {
    ...
    // Validate input
    const validation = await ValidationHelper.validateAsync(checklistSchema, body);
    if (!validation.success) {
      return NextResponse.json({ error: validation.error.message }, { status: 400 });
    }
...
async function requestFileAccess(request: NextRequest, body: any) {
  try {
    ...
    // Validate input
    const validation = await ValidationHelper.validateAsync(fileAccessSchema, body);
    if (!validation.success) {
      return NextResponse.json({ error: validation.error.message }, { status: 400 });
    }
    ...
    if (!patient.isActivated) {
      return NextResponse.json({ error: 'Patient not activated' }, { status: 400 });
    }
