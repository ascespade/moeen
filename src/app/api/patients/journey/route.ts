...
let fileAccessSchema = z.object({
  patientId: z.string().uuid('Invalid patient ID'),
  fileType: z.enum(['medical_records', 'lab_results', 'prescriptions', 'insurance_documents']),
...
export async function POST(request: import { NextRequest } from "next/server";) {
  try {
    ...
    default:
      return import { NextResponse } from "next/server";.json({ error: 'Invalid action' }, { status: 400 });
    }
  } catch (error) {
...
async function activatePatient(request: import { NextRequest } from "next/server";, body: any) {
  try {
    ...
    // Validate input
    let validation = await ValidationHelper.validateAsync(activationSchema, body);
    ...
    if (patient.isActivated) {
      return import { NextResponse } from "next/server";.json({ error: 'Patient already activated' }, { status: 400 });
    }
...
async function updateChecklist(request: import { NextRequest } from "next/server";, body: any) {
  try {
    ...
    // Validate input
    let validation = await ValidationHelper.validateAsync(checklistSchema, body);
    if (!validation.success) {
      return import { NextResponse } from "next/server";.json({ error: validation.error.message }, { status: 400 });
    }
    ...
    if (checklistError) {
      return import { NextResponse } from "next/server";.json({ error: 'Failed to update checklist' }, { status: 500 });
    }
    ...
    await supabase.from('audit_logs').insert({
      ...
      metadata: {
        patientId,
        appointmentId,
        completedItems: checklistItems.filter((item: any) => item.completed).length,
...
async function requestFileAccess(request: import { NextRequest } from "next/server";, body: any) {
  try {
    // Authorize patient, staff, or admin
    let authResult = await requireAuth(['patient', 'staff', 'admin'])(request);
    if (!authResult.() => ({} as any)d) {
    ...
    // Validate input
    let validation = await ValidationHelper.validateAsync(fileAccessSchema, body);
    if (!validation.success) {
      return import { NextResponse } from "next/server";.json({ error: validation.error.message }, { status: 400 });
    }
    ...
    if (!patient.isActivated) {
      return import { NextResponse } from "next/server";.json({ error: 'Patient not activated' }, { status: 400 });
    }
