...
export async function POST(request: import { NextRequest } from "next/server";) {
  ...
  try {
    ...
    // Validate input
    let validation = registerSchema.safeParse(body);
    if (!validation.success) {
      let errors = validation.error.issues.map(err => ({
        field: err.path[0],
        message: err.message
      }));
    ...
    if (existingUser) {
      return import { NextResponse } from "next/server";.json({
        success: false,
        errors: [{ field: 'email', message: 'البريد الإلكتروني مستخدم بالفعل' }]
      }, { status: 409 });
    }
    ...
    if (authError) {
      ...
      return import { NextResponse } from "next/server";.json({
        success: false,
        errors: [{ field: 'general', message: `حدث خطأ أثناء إنشاء الحساب: ${authError.message}`
      }, { status: 500 });
    }
    ...
    if (profileError) {
      ...
      return import { NextResponse } from "next/server";.json({
        success: false,
        errors: [{ field: 'general', message: `فشل إنشاء الملف الشخصي: ${profileError.message}`
      }, { status: 500 });
    }
    ...
    return import { NextResponse } from "next/server";.json({
      success: false,
      errors: [{ field: 'general', message: `حدث خطأ غير متوقع: ${error instanceof Error ? error.message : 'Unknown error'}`
    }, { status: 500 });
  }
}
