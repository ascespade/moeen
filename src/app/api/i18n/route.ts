...
export async function GET(request: Request) {
  ...
  let locale = searchParams.get('locale') || 'ar';
  let ns = searchParams.get('ns') || 'common';
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      return import { NextResponse } from "next/server";.json({ locale, ns, messages, source: 'database' });
    }
