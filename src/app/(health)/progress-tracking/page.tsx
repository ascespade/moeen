...
const ProgressTrackingPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'goals' | 'assessments' | 'reports'>('goals');
    ...
    <div className="container mx-auto px-4 py-8" dir="rtl">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">تتبع التقدم والتقييم</h1>
            <p className="text-gray-600 mt-2">مراقبة تقدم المرضى وتقييم أدائهم</p>
          </div>
          <Button 
              ...
              {filteredGoals.length === 0 ? (
                <Card>
                  <CardContent className="p-12 text-center">
                    ...
                    <p className="text-gray-600 mb-4">ابدأ بإنشاء هدف جديد للمريض</p>
                    <Button 
                      onClick={() => router.push('/progress-tracking/new')}
                      className="bg-[var(--brand-primary)] hover:brightness-95"
                    >
                    ...
                    <CardContent className="p-6">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              {goal.patients?.first_name} {goal.patients?.last_name} - {goal.patients?.condition}
                            </p>
                          </div>
                        </div>
                          ...
                          {goal.milestones.map((milestone) => (
                            <div key={milestone.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                              ...
                              <div className="text-right">
                                <p className="text-xs text-gray-600">{milestone.progress_percentage}%</p>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t">
                        <div className="text-sm text-gray-600">
                          الهدف حتى {new Date(goal.target_date).toLocaleDateString('ar-SA')}
                        </div>
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm">
              ...
              {assessments.length === 0 ? (
                <Card>
                  <CardContent className="p-12 text-center">
                    ...
                    <Button 
                      onClick={() => router.push('/progress-tracking/assessments/new')}
                      className="bg-[var(--brand-primary)] hover:brightness-95"
                    >
                      إنشاء تقييم جديد
                    </Button>
                ...
                assessments.map((assessment) => (
                  <Card key={assessment.id} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-lg font-semibold">{assessment.assessment_type}</h3>
                          <p className="text-sm text-gray-600">{assessment.category}</p>
                          <p className="text-xs text-gray-500 mt-1">
                            {assessment.assessors?.first_name} {assessment.assessors?.last_name}
                          </p>
                        </div>
                        <div className="text-right">
              ...
              {reports.length === 0 ? (
                <Card>
                  <CardContent className="p-12 text-center">
                    ...
                    <p className="text-gray-600 mb-4">سيتم إنشاء التقارير تلقائياً</p>
                  </CardContent>
                </Card>
              ) : (
                reports.map((report) => (
                  <Card key={report.id} className="hover:shadow-md transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-lg font-semibold">تقرير التقدم الشهري</h3>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t">
                        <div className="text-sm text-gray-600">
                          التقرير التالي: {new Date(report.next_review_date).toLocaleDateString('ar-SA')}
                        </div>
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm">
                            <Eye className="w-4 h-4 mr-1" />
