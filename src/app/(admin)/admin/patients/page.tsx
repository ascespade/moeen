...
function PatientsPageContent() {
  ...
  const getGenderBadge = (gender: string) => {
    return gender === 'male' ? (
      ...
      <Badge
        variant='outline'
        className='bg-[color-mix(in_srgb,var(--brand-primary)_10%,transparent)] text-[var(--brand-primary)] border-[color-mix(in_srgb,var(--brand-primary)_20%,transparent)]'
      >
        أنثى
      </Badge>
    );
      ...
      <div className='container-app py-6'>
              ...
              <div className='text-2xl font-bold'>
                {
                  patients.filter(p => {
                    ...
                    const today = new Date();
                    return (
                      lastVisitDate.toDateString() === today.toDateString()
                    );
                  }).length
                }
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>عرض وإدارة جميع مرضى المركز</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                        ...
                        {patient.bmi ? (
                          <div className='flex items-center gap-1'>
                            ...
                            {patient.bmi < 18.5 ? (
                              <Badge
                                variant='outline'
                                className='text-xs bg-[color-mix(in_srgb,var(--brand-info)_10%,transparent)] text-[var(--brand-info)] border-[color-mix(in_srgb,var(--brand-info)_20%,transparent)]'
                              >
                                نحيف
                              </Badge>
                            ) : patient.bmi < 25 ? (
                              <Badge
                                variant='outline'
                                className='text-xs bg-[color-mix(in_srgb,var(--brand-success)_10%,transparent)] text-[var(--brand-success)] border-[color-mix(in_srgb,var(--brand-success)_20%,transparent)]'
                              >
                                طبيعي
                              </Badge>
                            ) : patient.bmi < 30 ? (
                              <Badge
                                variant='outline'
                                className='text-xs bg-[color-mix(in_srgb,var(--brand-warning)_10%,transparent)] text-[var(--brand-warning)] border-[color-mix(in_srgb,var(--brand-warning)_20%,transparent)]'
                              >
                                زائد
                              </Badge>
                            ) : (
                              <Badge
                                variant='outline'
                                className='text-xs bg-[color-mix(in_srgb,var(--brand-error)_10%,transparent)] text-[var(--brand-error)] border-[color-mix(in_srgb,var(--brand-error)_20%,transparent)]'
                              >
                                سمنة
                              </Badge>
                            )}
            ...
            <div className='flex items-center justify-between mt-6'>
              <div className='text-sm text-muted-foreground'>
                ...
                {patients.length > 0
                  ...
                  : 0}{' '}
                - {Math.min(currentPageForPagination * 10, patients.length)} من{' '}
                {hookPagination?.totalItems || patients.length} مريض
              </div>
