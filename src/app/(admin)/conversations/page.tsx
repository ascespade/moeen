            ...
            {loading ? (
              ...
              <div className='grid gap-2'>
                {filtered.map(c => (
                  <button
                    key={c.id}
                    className={`text-start card card-interactive p-3 ${c.id === activeId ? 'bg-[var(--primary-primary)] text-white' : ''}`}
          ...
          <section className='card grid grid-rows-[1fr_auto]'>
            <div className='p-4 overflow-y-auto max-h-[60dvh] space-y-3'>
              {active?.messages.map(m => (
                <div
                  key={m.id}
                  className={`max-w-[85%] rounded-lg p-3 ${m.from === 'customer' ? 'bg-surface' : m.from === 'agent' ? 'bg-[var(--primary-accent)]/10' : 'bg-[var(--primary-success)]/10'}`}
                >
