      ...
      <div className="rounded-xl border p-4 grid gap-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
          ...
          <div className="flex items-center gap-2">
            <button className={`h-9 rounded-md border px-3 ${theme === "light" ? "bg-gray-900 text-white" : ""}`} onClick={() => setTheme("light")}>فاتح</button>
            <button className={`h-9 rounded-md border px-3 ${theme === "dark" ? "bg-gray-900 text-white" : ""}`} onClick={() => setTheme("dark")}>داكن</button>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
          <div className="text-sm">اللغة</div>
          <div className="flex items-center gap-2">
            <button className={`h-9 rounded-md border px-3 ${lang === "ar" ? "bg-gray-900 text-white" : ""}`} onClick={() => setLang("ar")}>العربية</button>
            <button className={`h-9 rounded-md border px-3 ${lang === "en" ? "bg-gray-900 text-white" : ""}`} onClick={() => setLang("en")}>English</button>
          </div>
        </div>
      </div>
