...
export default function OwnerDashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const monthRevenue = monthPayments?.reduce((sum, p) => sum + parseFloat(p.amount.toString()), 0) || 0;
      ...
      setStats({
        ...
        today_attendance_rate: todaySessions ? Math.round((completedToday || 0) / todaySessions * 100) : 0,
      ...
      const { data: recentSessions } = await supabase
        .from('appointments')
        .select('*, patient:patients(first_name), therapist:users!appointments_doctor_id_fkey(name)')
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="card p-6 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-green-700 dark:text-green-300">الإيرادات اليوم</p>
            <div className="text-3xl">💰</div>
          </div>
          <p className="text-3xl font-bold text-green-600 dark:text-green-400">
            {stats.today_revenue.toLocaleString()} <span className="text-xl">ريال</span>
          </p>
        </div>

        <div className="card p-6 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-blue-700 dark:text-blue-300">الجلسات اليوم</p>
            <div className="text-3xl">📅</div>
          </div>
        ...
        <div className="card p-6 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-purple-700 dark:text-purple-300">معدل الحضور</p>
            <div className="text-3xl">✅</div>
          </div>
        ...
        <div className="card p-6 bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-orange-700 dark:text-orange-300">مدفوعات معلقة</p>
            <div className="text-3xl">⏳</div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="card p-6">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
            {stats.month_revenue.toLocaleString()} <span className="text-sm">ريال</span>
          </p>
        </div>

        <div className="card p-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-gray-600 dark:text-gray-400">جلسات الأسبوع</p>
            <div className="text-2xl">📊</div>
          </div>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
            {stats.week_sessions}
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-gray-600 dark:text-gray-400">الأخصائيون</p>
