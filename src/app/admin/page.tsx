...
export default function AdminPage() {
  ...
  const getRoleBadge = (role: string) => {
    ...
    return <Badge className={colors[role as keyof typeof colors] || colors.demo}>{role}</Badge>;
  };
                ...
                {configs.map((config) => (
                  <div key={config.id} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {config.isSecret && <Key className="h-4 w-4 text-yellow-500" />}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        disabled={!editingConfig || editingConfig.value === config.value}
                      >
                        Save
                      </Button>
                    </div>
