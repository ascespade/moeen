...
export function useAdminPatients(): UseAdminPatientsReturn {
  ...
  const filteredPatients = patients.filter(patient => {
    ...
    const matchesAge =
      ...
      checkAgeRange(patient.date_of_birth, filters.ageRange);
  ...
  const createPatient = useCallback(
    async (patientData: Partial<Patient>): Promise<boolean> => {
      try {
  ...
  const updatePatient = useCallback(
    async (id: string, patientData: Partial<Patient>): Promise<boolean> => {
      try {
  ...
  const blockPatient = useCallback(
    async (id: string): Promise<boolean> => {
