...
export const useAuth = (): AuthState & AuthActions => {
  ...
  const loginWithCredentials = useCallback(
    async (email: string, password: string, rememberMe: boolean = false) => {
      try {
        logger.debug('[useAuth] loginWithCredentials request', {
          email,
          rememberMe,
        });
          ...
          try {
            ...
            if (
              ...
              if (demoRes.ok) {
                ...
                logger.debug('[useAuth] demo-email payload', demoData);
                if (
                  demoData.success &&
        ...
        try {
          logger.debug('[useAuth] fetching /api/auth/me after login');
          const controller = new AbortController();
          ...
          if (meRes.ok) {
            ...
            logger.debug('[useAuth] /api/auth/me after login payload', meData);
