...
export class HemamAssistant {
  ...
  private handleNewBeneficiaryFlow(__message: string): string {
...
3️⃣ معلومات عن خدماتنا`;
  ...
  private handleAppointmentFlow(__message: string): string {
...
4️⃣ إلغاء موعد`;
  ...
  private handleGeneralInquiry(__message: string): string {
...
4️⃣ التواصل مع الفريق الطبي`;
  ...
  // Family support flow
  private handleFamilySupportFlow(__message: string): string {
...
4️⃣ التواصل مع الطبيب`;
  ...
  // Default flow
  private handleDefaultFlow(__message: string): string {
