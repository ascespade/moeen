...
export class ErrorHandler {
  ...
  private determineSeverity(_error: Error): ErrorSeverity {
    const __message = error.message.toLowerCase();
  ...
  private async processErrorQueue(): Promise<void> {
    ...
    try {
      while (this.errorQueue.length > 0) {
        const __errorReport = this.errorQueue.shift();
        if (errorReport) {
  ...
  private async sendErrorReport(_errorReport: ErrorReport): Promise<void> {
    try {
...
export const __errorHandler = ErrorHandler.getInstance();
