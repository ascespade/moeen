...
export class SlackIntegration {
  ...
  private async loadChannels(): Promise<void> {
    try {
      ...
      if (response.ok && response.channels) {
        response.channels.forEach((_channel: unknown) => {
  ...
  private async loadUsers(): Promise<void> {
    try {
      ...
      if (response.ok && response.members) {
        response.members.forEach((_user: unknown) => {
  ...
  private async handleMessage(_event: unknown): Promise<void> {
    ...
    const __threadTs = event.thread_ts || event.ts;
    const __isInThread = event.thread_ts;
  ...
  // Handle app mentions
  private async handleAppMention(_event: unknown): Promise<void> {
    // Handle when the bot is mentioned
  }

  // Handle interactive messages (button clicks, etc.)
  private async handleInteractiveMessage(_event: unknown): Promise<void> {
  ...
  private async handleDoctorResponse(_event: unknown): Promise<void> {
