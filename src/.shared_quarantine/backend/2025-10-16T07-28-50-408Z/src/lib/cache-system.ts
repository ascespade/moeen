...
export class MemoryCache {
  ...
  set<T>(_key: string, data: T, ttl?: number): void {
    // Remove oldest entries if cache is full
    if (this.cache.size >= this.config.maxSize) {
      const __iter = this.cache.keys().next();
...
export class CacheKeys {
  static user(_id: string): string {
    return `user:${id}`;
  }

  static patient(_id: string): string {
    return `patient:${id}`;
  }

  static doctor(_id: string): string {
    return `doctor:${id}`;
  }
  ...
  static sessions(_patientId: string): string {
    return `sessions:${patientId}`;
  }

  static conversations(_patientId: string): string {
    return `conversations:${patientId}`;
  }

  static analytics(_period: string): string {
    return `analytics:${period}`;
  }

  static notifications(_userId: string): string {
    return `notifications:${userId}`;
  }

  static insuranceClaims(_patientId: string): string {
...
export class CacheManager {
  ...
  async getUser(_id: string, fetcher: () => Promise<any>): Promise<any> {
    return this.get(CacheKeys.user(id), fetcher, 10 * 60 * 1000); // 10 minutes
  }

  async getPatient(_id: string, fetcher: () => Promise<any>): Promise<any> {
    return this.get(CacheKeys.patient(id), fetcher, 5 * 60 * 1000); // 5 minutes
  }

  async getDoctor(_id: string, fetcher: () => Promise<any>): Promise<any> {
  ...
  async getCenterSettings(_fetcher: () => Promise<any>): Promise<any> {
    return this.get(CacheKeys.centerSettings(), fetcher, 30 * 60 * 1000); // 30 minutes
  }

  async getMessageTemplates(_fetcher: () => Promise<any>): Promise<any> {
    return this.get(CacheKeys.messageTemplates(), fetcher, 60 * 60 * 1000); // 1 hour
  }

  // Cache invalidation methods
  invalidateUser(_id: string): void {
  ...
  invalidatePatient(_id: string): void {
  ...
  invalidateDoctor(_id: string): void {
  ...
  invalidateConversations(_patientId: string): void {
  ...
  invalidateInsuranceClaims(_patientId: string): void {
...
export const __cache = CacheManager.getInstance();
