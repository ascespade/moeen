...
class Logger {
  ...
  private shouldLog(_level: LogLevel): boolean {
    ...
    const __currentLevelIndex = levels.indexOf(this.logLevel);
  ...
  private writeLog(_entry: LogEntry): void {
    if (!this.shouldLog(entry.level)) return;

    const __formattedLog = this.formatLog(entry);
...
export const __logError = logger.error.bind(logger);
export const __logWarn = logger.warn.bind(logger);
export const __logInfo = logger.info.bind(logger);
export const __logDebug = logger.debug.bind(logger);
export const __logTrace = logger.trace.bind(logger);
