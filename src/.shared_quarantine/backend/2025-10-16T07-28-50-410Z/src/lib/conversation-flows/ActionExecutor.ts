...
export class ActionExecutor {
  ...
  private async sendNotification(_data: unknown, context: ConversationContext) {
    const {
      type,
      patientId,
      channels = ["email"],
  ...
  private async updatePatient(_data: unknown, context: ConversationContext) {
  ...
  private async sendEmail(_data: unknown, context: ConversationContext) {
    const { to, template, templateData } = data;

    const __result = await emailService.sendEmail({
      to,
      template,
      data: templateData,
      language: "ar",
  ...
  private async sendSMS(_data: unknown, context: ConversationContext) {
