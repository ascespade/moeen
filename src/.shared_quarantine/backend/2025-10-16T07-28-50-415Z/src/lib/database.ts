...
export class DatabaseManager {
  ...
  async createPatient(_patientData: {
    ...
    const __result = await this.pool.query(query, values);
    return result.rows[0];
  }

  async getPatient(_patientId: number) {
    const __query = "SELECT * FROM patients WHERE id = $1";
    const __result = await this.pool.query(query, [patientId]);
    return result.rows[0];
  }

  async getPatientByPhone(_phone: string) {
    const __query = "SELECT * FROM patients WHERE phone = $1";
    const __result = await this.pool.query(query, [phone]);
    return result.rows[0];
  }

  async updatePatient(_patientId: number, updates: unknown) {
    const __fields = Object.keys(updates);
    const __values = Object.values(updates);
    const __setClause = fields
    ...
    const __result = await this.pool.query(query, [patientId, ...values]);
    return result.rows[0];
  }
  ...
  async getPatientStats() {
    ...
    const __result = await this.pool.query(query);
    return result.rows[0];
  }
