...
export class FlowManager {
  ...
  // Get flow by ID
  getFlow(_flowId: string): ConversationFlow | undefined {
    return this.flows.get(flowId);
  }

  // Get flow by entry point
  getFlowByEntryPoint(_entryPoint: string): ConversationFlow | undefined {
  ...
  private async executeSlackNotification(
    ...
    try {
      const __response = await fetch("/api/slack/notify", {
        method: "POST",
        headers: {
  ...
  // Add new flow
  addFlow(_flow: ConversationFlow): void {
    this.flows.set(flow.id, flow);
  }

  // Update existing flow
  updateFlow(_flowId: string, flow: ConversationFlow): void {
    this.flows.set(flowId, flow);
  }

  // Delete flow
  deleteFlow(_flowId: string): void {
