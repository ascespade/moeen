...
export class SaudiMinistryHealthIntegration {
  ...
  async syncWithShoon(
    ...
    try {
      const __shoonResponse = await this.callShoonAPI(
        "GET",
        `/patients/${nationalId}`,
      );
  ...
  async getShoonPatientData(_nationalId: string): Promise<any> {
    try {
      const __response = await this.callShoonAPI(
        "GET",
  ...
  async syncWithTatman(
    ...
    try {
      const __tatmanResponse = await this.callTatmanAPI(
        "GET",
  ...
  async verifyTatmanCoverage(
    ...
    try {
      const __response = await this.callTatmanAPI("POST", "/verify-coverage", {
        nationalId,
        serviceCode,
  ...
  // Utility Functions
  private mapSehaToHealthRecord(_sehaData: unknown): MinistryHealthRecord {
    return {
  ...
  private isValidSaudiAddress(_address: unknown): boolean {
    return !!(address.city && address.district && address.street);
  }

  private isValidInsuranceData(_insurance: unknown): boolean {
    return !!(
      insurance.provider &&
  ...
  private isValidMedicalHistory(_medicalHistory: unknown): boolean {
    return (
...
export const __ministryHealthIntegration = new SaudiMinistryHealthIntegration();
