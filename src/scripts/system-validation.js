...
validator.addCheck('Required directories exist', () => {
  ...
  if (missing.length > 0) {
    return { passed: false, message: `Missing directories: ${missing.join(', ')}` };
  }
...
validator.addCheck('Critical API routes exist', () => {
  ...
  if (missing.length > 0) {
    return { passed: false, message: `Missing API routes: ${missing.join(', ')}` };
  }
...
validator.addCheck('No logger.info in production files', () => {
...
validator.addCheck('TypeScript files are valid', () => {
  ...
  tsFiles.forEach(file => {
    try {
      ...
      if (content.includes('any: any') || content.includes('undefined: undefined')) {
...
validator.addCheck('Environment variables are configured', () => {
  ...
  if (missing.length > 0) {
    return { passed: false, message: `Missing environment variables: ${missing.join(', ')}` };
  }
