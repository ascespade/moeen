...
async function seedTranslations() {
  try {
    ...
    const arabicTranslations = flatTranslations.map(translation => ({
      lang_code: 'ar',
      key: translation.key,
    ...
    const englishTranslations = flatTranslations.map(translation => ({
      lang_code: 'en',
      key: translation.key,
      value: translation.key.split('.').pop() || translation.value
    }));
    
    const allTranslations = [...arabicTranslations, ...englishTranslations];
    
    logger.info(`📊 Total translations to insert: ${allTranslations.length}`);
    ...
    for (let i = 0; i < allTranslations.length; i += batchSize) {
      ...
      if (error) {
        ...
        logger.info(`✅ Inserted batch ${i / batchSize + 1}/${Math.ceil(allTranslations.length / batchSize)}`);
      }
    }
