...
async function __seedTranslations() {
  ...
  try {
    ...
    function __flattenKeys(
      ...
      for (const [key, value] of Object.entries(obj)) {
        const __fullKey = prefix ? `${prefix}.${key}` : key;
    ...
    const __flatTranslations = flattenKeys(translationKeys);
    ...
    const __batchSize = 100;
    for (let i = 0; i < translations.length; i += batchSize) {
      const __batch = translations.slice(i, i + batchSize);
