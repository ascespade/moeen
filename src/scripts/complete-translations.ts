...
async function seedTranslations() {
  ...
  try {
    ...
    function flattenKeys(obj: any, prefix = ''): Array<{key: string, value: string}> {
    ...
    for (const translation of flatTranslations) {
      const englishValue = translation.key.split('.').pop() || translation.value;
      translations.push({
        lang_code: 'en',
        key: translation.key,
        value: englishValue
    ...
    for (let i = 0; i < translations.length; i += batchSize) {
      ...
      if (error) {
        ...
        logger.info(`✅ Inserted batch ${i / batchSize + 1}/${Math.ceil(translations.length / batchSize)}`);
      }
    }
