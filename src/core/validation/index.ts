...
import { _REGEX_PATTERNS } from "../constants";
import { _ValidationError } from "../errors";

// Base Validation Schemas
export const __baseSchemas = {
...
export const __appointmentSchemas = {
  create: z.object({
...
export const __insuranceSchemas = {
  create: z.object({
    patientId: baseSchemas.id,
    appointmentId: baseSchemas.id,
...
export const __fileUploadSchemas = {
  upload: z.object({
...
export const __notificationSchemas = {
  send: z.object({
    userId: baseSchemas.id,
    type: z.enum([
...
export class ValidationHelper {
  public static validate<T>(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        const __validationError = new ValidationError(
          error.errors
  ...
  > {
    try {
      const __result = await schema.parseAsync(data);
...
export const __validateRequest = <T>(
  schema: z.ZodSchema<T>,
  context?: Record<string, any>,
) => {
  return (_req: unknown, res: unknown, next: unknown) => {
...
export const __validateQuery = <T>(
  schema: z.ZodSchema<T>,
  context?: Record<string, any>,
) => {
  return (_req: unknown, res: unknown, next: unknown) => {
    const __searchParams = new URLSearchParams(req.url.split("?")[1] || "");
    const __validation = ValidationHelper.validateQueryParams(
      schema,
      searchParams,
      context,
    );
